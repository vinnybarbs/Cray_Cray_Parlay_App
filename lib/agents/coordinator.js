...
class MultiAgentCoordinator {
  ...
  async gatherRealDataForAI(games, request) {
    ...
    try {
      ...
      if (Object.values(realData.recentNews).flat().length > 0) {
        const sampleArticles = Object.values(realData.recentNews).flat().slice(0, 2);
        console.log(`  Sample News:`, sampleArticles.map(a => `${a.title.substring(0, 50)}...`).join(' | '));
      }
