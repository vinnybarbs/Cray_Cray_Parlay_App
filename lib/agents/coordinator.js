...
class MultiAgentCoordinator {
  ...
  let aiContent = '';
  let attempts = 0;
  const maxAttempts = request.riskLevel === 'Low' ? 2 : 1; // allow one retry for Low-risk odds policy
      ...
      while (attempts < maxAttempts) {
        ...
        if (request.riskLevel === 'Low') {
          ...
          if (violations > 0) {
            ...
            lastConflictSummary = `LOW-RISK ODDS POLICY VIOLATIONS: ${violations} leg(s) had odds not heavy-favorite (found: ${found.slice(0,6).join(', ')}). For Low risk, each leg should be a heavy favorite between -200 and -1000 (or shorter). Replace with safer legs and keep variety rules.`;
