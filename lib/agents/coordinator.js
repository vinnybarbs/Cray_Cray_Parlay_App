...
class MultiAgentCoordinator {
  ...
  const enrichedGames = await this.researchAgent.deepResearch(oddsResult.odds, { 
    ...
    tResearchMs = Date.now() - tResearch0;
      ...
      if (sport && sportsWithPlayerProps.includes(sport) && process.env.API_SPORTS_KEY) {
      ...
      } else if (sport && sportsWithPlayerProps.includes(sport) && !process.env.API_SPORTS_KEY) {
        console.log('⚠️ API_SPORTS_KEY not configured - skipping player verification');
      }
  ...
  async healthCheck() {
    const health = {
      ...
      rosterCache: !!process.env.API_SPORTS_KEY
    };
