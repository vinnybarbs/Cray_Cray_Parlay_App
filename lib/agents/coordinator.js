...
class MultiAgentCoordinator {
  ...
  async gatherRealDataForAI(games, request) {
    ...
    try {
      // 0. Get API-Sports data (injuries & records) - NEW!
      console.log(`  🏈 Fetching API-Sports data...`);
      const ApiSportsResearch = require('../services/apisports-research');
      const apiSportsResearch = new ApiSportsResearch();
      
      const teamNames = [...new Set(games.map(g => [g.home_team, g.away_team]).flat())];
      
      // Get injuries from API-Sports
      const apiInjuries = await apiSportsResearch.getInjuriesForGames(games);
      realData.injuries = apiInjuries;
      console.log(`    ✓ Found injuries for ${Object.keys(apiInjuries).length} teams`);
      
      // Get records from API-Sports (preferred over old team_stats_season)
      const apiRecords = await apiSportsResearch.getTeamRecords(teamNames);
      if (Object.keys(apiRecords).length > 0) {
        Object.entries(apiRecords).forEach(([team, record]) => {
          realData.teamRecords[team] = record;
        });
        console.log(`    ✓ Found API-Sports records for ${Object.keys(apiRecords).length} teams`);
      }

      // Get recent team stats (last 3 games)
      const teamStats = await apiSportsResearch.getTeamRecentStats(teamNames, 3);
      if (Object.keys(teamStats).length > 0) {
        realData.teamStats = teamStats;
        console.log(`    ✓ Found recent stats for ${Object.keys(teamStats).length} teams`);
      }

      // Get top performers
      const topPerformers = await apiSportsResearch.getTopPerformers(teamNames);
      if (Object.keys(topPerformers).length > 0) {
        realData.topPerformers = topPerformers;
        console.log(`    ✓ Found top performers for ${Object.keys(topPerformers).length} teams`);
      }
      
      // 1. Get team W-L records from Supabase (fallback if API-Sports data not available)
      if (this.supabase) {
