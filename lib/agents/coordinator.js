...
class MultiAgentCoordinator {
  ...
  stripParlayJson(text) {
    try {
      ...
      // Also handle incomplete JSON blocks (missing END marker) - remove from BEGIN to end of text or next section
      const incompleteRe = /===BEGIN_PARLAY_JSON===([\s\S]*?)(?=\*\*🔒|$)/g;
      cleaned = cleaned.replace(incompleteRe, '');
