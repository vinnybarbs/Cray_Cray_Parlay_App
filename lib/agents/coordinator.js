        ...
        try {
          ...
          if (preAnalyses && preAnalyses.length > 0) {
            ...
            for (const a of preAnalyses) {
              const key = `${a.away_team}_${a.home_team}`;
              analysisMap[key] = a;
              // Also try reversed key in case team order differs
              const reverseKey = `${a.home_team}_${a.away_team}`;
              analysisMap[reverseKey] = a;
            }
            ...
            for (const game of enrichedGames) {
              ...
              if (analysis) {
                // Build rich research text from pre-analysis
                const parts = [];
                ...
                // Prepend to existing research or replace empty research
            ...
            console.log(`🧠 Pre-analysis enriched ${enriched}/${enrichedGames.length} games with AI analysis`);
          }
        } catch (err) {
