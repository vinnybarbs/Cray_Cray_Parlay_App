...
class MultiAgentCoordinator {
  ...
  async gatherRealDataForAI(games, request) {
    ...
    try {
      ...
      if (this.supabase) {
        // Get CURRENT NFL season (not calendar year!)
        // NFL season runs Sep-Feb, so if we're before March, use last year
        const now = new Date();
        const currentMonth = now.getMonth(); // 0-11
        const currentSeason = (currentMonth < 2) ? now.getFullYear() - 1 : now.getFullYear();
        ...
        console.log(`  ✅ Loaded records for ${Object.keys(realData.teamRecords).length} teams`);
      } else {
      ...
      if (this.supabase) {
        ...
        for (const teamName of teamNames) {
          const { data: articles, error: newsError } = await this.supabase
            .from('news_articles')
            .select('title, content, published_at, source_name')
