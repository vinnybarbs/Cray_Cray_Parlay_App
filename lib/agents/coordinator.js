...
class MultiAgentCoordinator {
  constructor(fetcher, apiKeys, supabase = null) {
    ...
    // Initialize agents (pass supabase to odds agent for caching)
  ...
  const enrichedGames = await this.researchAgent.deepResearch(oddsResult.odds, { 
    ...
    tResearchMs = Date.now() - tResearch0;
      ...
      let filteredGames;
      try {
        filteredGames = filterMarketsByBetTypes(enrichedGames, request.selectedBetTypes, request.riskLevel);
