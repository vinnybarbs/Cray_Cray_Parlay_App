...
const { sportDayISO, daysAgo } = require('../services/sport-day.js');
...
class MultiAgentCoordinator {
  ...
  const enrichedGames = await this.researchAgent.deepResearch(oddsResult.odds, { 
    ...
    tResearchMs = Date.now() - tResearch0;
      ...
      // Phase 2.1: Enrich game research with pre-computed game_analysis
      await this.enrichGamesWithPreAnalysis(enrichedGames);
      ...
      const sportsWithPlayerProps = ['NFL', 'NCAAF', 'NBA', 'NCAAB'];
      
      // Team-name validation runs off the supabase team cache, no external API.
      if (sport && sportsWithPlayerProps.includes(sport)) {
  ...
  let aiContent = '';
  let attempts = 0;
  // Low risk gets two retries for the odds policy. Everything else gets one
  // retry so conflict and structure violations can be fed back to the model.
  const maxAttempts = request.riskLevel === 'Low' ? 3 : 2;
    const tAnalysis0 = Date.now();
      ...
      while (attempts < maxAttempts) {
        ...
        aiContent = await this.analyst.generateParlayWithAI(prompt);
        ...
        // Structure check: legs must spread across different games. Same-game
        // stacking is a thin-slate fallback, never the default shape.
        const diversity = this.validateGameDiversity(aiContent, request.numLegs, filteredGames.length);
        if (!diversity.ok) {
          const msg = `PARLAY STRUCTURE VIOLATION: ${diversity.reason} ACTION: Rebuild with one leg per game. Only take a second leg from a game when there are fewer games than legs, and never more than 2 legs from one game.`;
          lastConflictSummary = lastConflictSummary ? `${lastConflictSummary}\n${msg}` : msg;
          console.log('⚠️ ' + msg);
        }

        // Enforce Easy Money / Low-risk policy: moneyline favorites only (-150 or shorter)
        ...
        if (request.riskLevel === 'Low') {
          ...
          while ((m = re.exec(aiContent))) {
            ...
            if (Number.isNaN(val) || val > -150) { // Easy Money: must be -150 or stronger
              violations++;
              found.push(raw);
            }
          }
          if (violations > 0) {
            ...
            lastConflictSummary = `EASY MONEY POLICY: ${violations} leg(s) are not moneyline favorites (found: ${found.slice(0,6).join(', ')}). ACTION: Use ONLY moneyline favorites with odds -150 to -1000. NO spreads, NO totals, NO props. Pick the team most likely to WIN outright.`;
        ...
        // Only reject on TRUE conflicts, structure, or policy issues. Be lenient on leg count (close is good enough)
        const legCountClose = Math.abs(legCount - request.numLegs) <= 1;
        const pass = (jsonOk || legCountClose) && !hasConflicts && !hasPolicyIssues && diversity.ok;
        console.log(`📊 Validation: JSON ${jsonOk ? 'OK' : 'FAIL'}, Text legs ${legCount}/${request.numLegs}, Conflicts: ${hasConflicts ? 'YES' : 'NO'}, Game spread: ${diversity.ok ? 'OK' : 'FAIL'}, Low-risk policy: ${hasPolicyIssues ? 'FAIL' : 'OK'}`);
  ...
  const validationResult = this.validateParlayContent(finalContent, enrichedGames);
      ...
      const metadata = {
        ...
        phases: {
          ...
          analysis: { complete: true, attempts: attempts, model: 'anthropic' },
  ...
  // The house parlay structure is one leg per game. Two legs from one game
  // are tolerated only on thin slates (fewer games than requested legs), and
  // a parlay stacked on a single game is never acceptable when more games
  // are available.
  validateGameDiversity(content, numLegs, gamesAvailable) {
    let games = [];
    const jsonBlock = this.extractParlayJson(content);
    if (jsonBlock) {
      try {
        const parsed = JSON.parse(jsonBlock);
        games = (parsed?.parlay?.legs || []).map(l => (l.game || '').trim()).filter(Boolean);
      } catch { /* fall through to text parse */ }
    }
    if (games.length === 0) {
      // Text fallback: read Game: lines from the main parlay only, the lock
      // parlay repeats legs and would double count.
      let inLock = false;
      for (const line of content.split('\n')) {
        if (/LOCK\s+PARLAY:/i.test(line)) inLock = true;
        if (inLock) continue;
        const m = line.match(/Game:\s*(.+)/);
        if (m) games.push(m[1].trim());
      }
    }
    if (games.length === 0) return { ok: true, reason: '' };

    const counts = new Map();
    games.forEach(g => counts.set(g, (counts.get(g) || 0) + 1));
    const maxPerGame = gamesAvailable >= numLegs
      ? 1
      : Math.max(2, Math.ceil(numLegs / Math.max(gamesAvailable, 1)));
    const offenders = [...counts.entries()].filter(([, n]) => n > maxPerGame);
    if (offenders.length === 0) return { ok: true, reason: '' };
    const list = offenders.map(([g, n]) => `"${g}" has ${n} legs`).join(', ');
    return {
      ok: false,
      reason: `${list} (max ${maxPerGame} leg(s) per game with ${gamesAvailable} games available).`
    };
  }
  ...
  async generatePickSuggestions(request) {
    ...
    try {
      // Phase 0: Fetch historical lessons from learning system
      console.log('\n🧠 Fetching historical lessons...');
      const LearningAnalyzer = require('../services/learning-analyzer');
      const analyzer = new LearningAnalyzer();
      
      const historicalLessons = await analyzer.getRelevantLessons({
        sport: request.sports[0], // Primary sport
        limit: 10
      });
      
      console.log(`  Found ${historicalLessons.length} relevant lessons from past outcomes`);
      ...
      if (!oddsResult || !oddsResult.odds || oddsResult.odds.length === 0) {
        throw new Error('No odds found in the selected date range for your sport(s). Try extending the date range or choosing a different sport.');
      }
      ...
      // Phase 2.1: Enrich with pre-computed game_analysis (same as parlay flow)
      await this.enrichGamesWithPreAnalysis(enrichedGames);
      ...
      console.log(`✅ Extracted ${allPicks.length} possible picks (includes all bet sides - AI will compare and select best)`);

      // Phase 4.1: Math-grade every pick using pre-analyzed edges. Each pick
      // gains signedEdge / edgePp / isMathPick so the analyst-agent's picks
      // menu can surface "+12.4pp ✅" annotations and the LLM stops choosing
      // negative-edge sides.
      const pickGrader = require('../services/pick-grader');
      const gameByKey = new Map();
      for (const g of filteredGames) {
        gameByKey.set(`${g.away_team}_${g.home_team}`, g);
      }
      let gradedCount = 0;
      for (const pick of allPicks) {
        const game = gameByKey.get(`${pick.awayTeam}_${pick.homeTeam}`);
        const pre = game?._preAnalysis;
        if (!pre?.edges) continue;
        pickGrader.annotatePicksWithEdges([pick], { home_team: pick.homeTeam, away_team: pick.awayTeam }, pre.edges, pre.recommended_side);
        if (pick.signedEdge != null) gradedCount++;
      }
      console.log(`📐 Math-graded ${gradedCount}/${allPicks.length} picks with per-side edges`);
      
      // Show breakdown
      const picksByType = allPicks.reduce((acc, pick) => {
        acc[pick.betType] = (acc[pick.betType] || 0) + 1;
        return acc;
      }, {});
      console.log(`  📊 Breakdown:`, picksByType);
      ...
      // Phase 5: AI ranks and selects best picks WITH REAL DATA + HISTORICAL LESSONS + FUNCTION CALLING
      const selectedPicks = await this.analyst.selectBestPicks({
        picks: allPicks,
        numSuggestions: request.numSuggestions,
        riskLevel: request.riskLevel,
        betTypes: request.betTypes,
        apiKey: null, // analyst uses the shared Anthropic client (env key)
        realData: realData, // PASS REAL DATA TO PREVENT HALLUCINATIONS
        historicalLessons: historicalLessons, // PASS LESSONS FROM PAST OUTCOMES
        supabase: this.supabase // ENABLE FUNCTION CALLING - AI CAN QUERY DB DYNAMICALLY
      });
  ...
  extractIndividualPicks(games, request) {
    ...
    console.log(`📦 Extracting ALL possible picks from ${games.length} games (no pre-filtering)...`);
    
    for (const game of games) {
      const bookmaker = game.bookmakers?.[0];
      if (!bookmaker) continue;
      
      // Extract ALL outcomes from ALL markets - let AI decide which have value
      for (const market of bookmaker.markets || []) {
        const marketType = market.key;
        
        // For traditional bets (h2h, spreads, totals): Pass ALL outcomes to AI
        if (marketType === 'h2h' || marketType === 'spreads' || marketType === 'totals') {
          for (const outcome of market.outcomes || []) {
            picks.push(this.createPick(game, market, outcome, request));
          }
        }
        // For player props: Pass first 10 outcomes per market (avoid overwhelming AI with 100+ props per game)
        else if (marketType.includes('player')) {
          const propOutcomes = (market.outcomes || []).slice(0, 10);
    ...
    console.log(`  ✅ Extracted ${picks.length} total picks (AI will see all options)`);
    return picks;
  }
  ...
  createPick(game, market, outcome, request) {
    ...
    return {
      ...
      research: [
        game.research || '',
        game.intelligenceContext?.context || '',
        game.preAnalysis?.analysis_snippet ? `PRE-ANALYSIS: ${game.preAnalysis.analysis_snippet}` : ''
      ].filter(Boolean).join('\n'),
      gameContext: {
  ...
  getSuggestedSports(currentSports) {
    const allSports = ['NFL', 'NBA', 'NHL', 'Soccer', 'MLB', 'NCAAF', 'NCAAB'];
  ...
  async enrichWithCachedIntelligence(games, selectedSports) {
    try {
      ...
      for (const game of games) {
        ...
        // Map sport_key slug to friendly name for whitelist comparison
        const slugToFriendly = {
          'AMERICANFOOTBALL_NFL': 'NFL', 'BASKETBALL_NBA': 'NBA', 'BASKETBALL_NCAAB': 'NCAAB',
          'AMERICANFOOTBALL_NCAAF': 'NCAAF', 'BASEBALL_MLB': 'MLB', 'ICEHOCKEY_NHL': 'NHL',
          'SOCCER_EPL': 'SOCCER'
        };
        const friendlySport = slugToFriendly[sport] || sport;

        if (!selectedSports.includes(friendlySport) || !['NFL', 'NBA', 'NCAAB', 'NCAAF', 'MLB', 'NHL', 'SOCCER'].includes(friendlySport)) {
        ...
        try {
          ...
          const intelligence = await this.intelligenceService.getAgentContext(friendlySport, homeTeam, awayTeam);
    ...
    try {
      ...
      for (const game of games) {
        ...
        const slugToFriendly = {
          'AMERICANFOOTBALL_NFL': 'NFL', 'BASKETBALL_NBA': 'NBA', 'BASKETBALL_NCAAB': 'NCAAB',
          'AMERICANFOOTBALL_NCAAF': 'NCAAF', 'BASEBALL_MLB': 'MLB', 'ICEHOCKEY_NHL': 'NHL',
          'SOCCER_EPL': 'SOCCER', 'GOLF_PGA': 'GOLF', 'TENNIS_ATP': 'TENNIS', 'MMA_UFC': 'UFC'
        };
        const friendlySport = slugToFriendly[sport] || sport;

        // Only enrich supported sports with cached data
        if (!selectedSports.includes(friendlySport) || !['NFL', 'NBA', 'NCAAB', 'NCAAF', 'MLB', 'NHL', 'SOCCER', 'GOLF', 'TENNIS', 'UFC'].includes(friendlySport)) {
          enrichedGames.push(enrichedGame);
          continue;
        }
        ...
        try {
          ...
          const matchupContext = await this.statsService.getMatchupContext(friendlySport, homeTeam, awayTeam);
  ...
  /**
   * Enrich game objects with pre-computed analysis from game_analysis table.
   * Mutates game.research in place. Used by both parlay and suggestion flows.
   */
  async enrichGamesWithPreAnalysis(games) {
    if (!this.supabase || !games || games.length === 0) return;
    try {
      console.log(`  🧠 Fetching pre-computed game analysis for ${games.length} games...`);
      const { data: preAnalyses } = await this.supabase
        .from('game_analysis')
        .select('home_team, away_team, analysis_snippet, edge_score, recommended_pick, recommended_side, key_factors, spread, total, home_record, away_record, home_ranking, away_ranking, news_context, injury_context, edges')
        .eq('stale', false)
        .gte('expires_at', new Date().toISOString());

      if (!preAnalyses || preAnalyses.length === 0) {
        console.log(`    ⚠️ No fresh game analysis found`);
        return;
      }

      const analysisMap = {};
      for (const a of preAnalyses) {
        analysisMap[`${a.away_team}_${a.home_team}`] = a;
        analysisMap[`${a.home_team}_${a.away_team}`] = a;
      }

      let enriched = 0;
      for (const game of games) {
        const key1 = `${game.away_team}_${game.home_team}`;
        const key2 = `${game.home_team}_${game.away_team}`;
        const analysis = analysisMap[key1] || analysisMap[key2];
        if (!analysis) continue;

        const parts = [];
        if (analysis.analysis_snippet) parts.push(analysis.analysis_snippet);
        if (analysis.home_record || analysis.away_record) {
          const homeRec = analysis.home_record ? `${game.home_team} ${analysis.home_record}` : game.home_team;
          const awayRec = analysis.away_record ? `${game.away_team} ${analysis.away_record}` : game.away_team;
          const homeRank = analysis.home_ranking ? ` (#${analysis.home_ranking})` : '';
          const awayRank = analysis.away_ranking ? ` (#${analysis.away_ranking})` : '';
          parts.push(`Records: ${awayRec}${awayRank} vs ${homeRec}${homeRank}`);
        }
        if (analysis.spread) parts.push(`Spread: ${game.home_team} ${analysis.spread}`);
        if (analysis.total) parts.push(`O/U: ${analysis.total}`);
        if (analysis.key_factors && analysis.key_factors.length > 0) {
          parts.push(`Key factors: ${analysis.key_factors.join(', ')}`);
        }
        if (analysis.recommended_pick) parts.push(`AI Edge (${analysis.edge_score}/10): ${analysis.recommended_pick} [${analysis.recommended_side}]`);
        if (analysis.news_context) parts.push(`News: ${analysis.news_context}`);
        if (analysis.injury_context) parts.push(`Injuries: ${analysis.injury_context}`);

        const preResearch = parts.join('. ');

        if (!game.research || game.research.trim() === '' || game.research === 'No research data available') {
          game.research = preResearch;
        } else {
          game.research = preResearch + ' | ADDITIONAL: ' + game.research;
        }

        // Stash structured pre-analysis (edges, math pick) for downstream
        // pick annotation. String `research` is for the LLM brief; this
        // object lets pickGrader attach signed edges to each odds row.
        game._preAnalysis = {
          edges: analysis.edges || null,
          recommended_side: analysis.recommended_side || null,
          recommended_pick: analysis.recommended_pick || null,
          edge_score: analysis.edge_score != null ? Number(analysis.edge_score) : null,
        };

        enriched++;
      }
      console.log(`🧠 Pre-analysis enriched ${enriched}/${games.length} games with AI analysis`);
    } catch (err) {
      console.log(`⚠️ Pre-analysis enrichment failed:`, err.message);
    }
  }
  ...
  async gatherRealDataForAI(games, request) {
    ...
    try {
      ...
      // 2. Get PRE-COMPUTED news insights (not raw articles!)
      // These are summarized by background job into betting-relevant facts
      const NewsSummarizer = require('../services/news-summarizer');
      const newsSummarizer = new NewsSummarizer();
      
      const newsInsights = await newsSummarizer.getTeamInsights(teamNames, 7);
      
      if (Object.keys(newsInsights).length > 0) {
        realData.newsInsights = newsInsights;
        console.log(`  ✅ Loaded pre-computed insights for ${Object.keys(newsInsights).length} teams`);
      } else {
        console.log(`  ⚠️ No pre-computed news insights available (run summarization job)`);
      }

      // 2.5 INJECT PRE-COMPUTED GAME ANALYSIS (from game_analysis table)
      // This fills gaps for NCAAB/NBA/NHL where API-Sports data is missing
      if (this.supabase) {
        try {
          console.log(`  🧠 Fetching pre-computed game analysis...`);
          const { data: analyses, error: analysisErr } = await this.supabase
            .from('game_analysis')
            .select('*')
            .eq('stale', false)
            .gte('expires_at', new Date().toISOString())
          ...
          if (!analysisErr && analyses && analyses.length > 0) {
            realData.gameAnalysis = analyses;
            console.log(`    ✓ Loaded ${analyses.length} pre-analyzed games`);

            // Fill missing team records from pre-analysis
            for (const a of analyses) {
              if (a.home_team && a.home_record && !realData.teamRecords[a.home_team]) {
                realData.teamRecords[a.home_team] = { record: a.home_record, wins: 0, losses: 0 };
                const parts = a.home_record.match(/(\d+)-(\d+)/);
                if (parts) {
                  realData.teamRecords[a.home_team].wins = parseInt(parts[1]);
                  realData.teamRecords[a.home_team].losses = parseInt(parts[2]);
                }
                if (a.home_ranking) realData.teamRecords[a.home_team].ranking = a.home_ranking;
              }
              if (a.away_team && a.away_record && !realData.teamRecords[a.away_team]) {
                realData.teamRecords[a.away_team] = { record: a.away_record, wins: 0, losses: 0 };
                const parts = a.away_record.match(/(\d+)-(\d+)/);
                if (parts) {
                  realData.teamRecords[a.away_team].wins = parseInt(parts[1]);
                  realData.teamRecords[a.away_team].losses = parseInt(parts[2]);
                }
                if (a.away_ranking) realData.teamRecords[a.away_team].ranking = a.away_ranking;
              }

              // Fill news insights from pre-analysis news_context
              if (a.news_context) {
                if (!realData.newsInsights) realData.newsInsights = {};
                if (!realData.newsInsights[a.home_team]) {
                  realData.newsInsights[a.home_team] = {
                    insights: a.news_context,
                    sentiment: 'neutral',
                    injuries: []
                  };
                }
              ...
              // Fill injury context
              if (a.injury_context) {
                if (!realData.injuries[a.home_team]) {
                  realData.injuries[a.home_team] = a.injury_context.split(',').map(i => {
                    const trimmed = i.trim();
                    return { player: trimmed, position: '', status: 'Listed', injury: '' };
                  });
                }
              }
            }
            console.log(`    ✓ Enriched ${Object.keys(realData.teamRecords).length} team records from pre-analysis`);
          } else {
            console.log(`    ⚠️ No fresh game analysis found`);
          }

          // Also fetch recent game_results for W-L trends (Eastern game-days,
          // matching how game_results.date is bucketed)
          const threeDaysAgo = sportDayISO(daysAgo(3));
          const { data: recentGames, error: recentErr } = await this.supabase
            .from('game_results')
          ...
          if (!recentErr && recentGames && recentGames.length > 0) {
            // Build recent form data from actual game results
            if (!realData.teamStats) realData.teamStats = {};
            const teamScores = {};
            for (const g of recentGames) {
              for (const [teamName, score, oppScore] of [
                [g.home_team_name, g.home_score, g.away_score],
                [g.away_team_name, g.away_score, g.home_score]
              ]) {
                if (!teamName) continue;
                if (!teamScores[teamName]) teamScores[teamName] = [];
                teamScores[teamName].push({ scored: score, allowed: oppScore });
              }
            }
            for (const [team, scores] of Object.entries(teamScores)) {
              if (scores.length >= 2 && !realData.teamStats[team]) {
                const avgPts = scores.reduce((s, g) => s + g.scored, 0) / scores.length;
                const avgAllowed = scores.reduce((s, g) => s + g.allowed, 0) / scores.length;
                realData.teamStats[team] = {
                  averages: { points: Math.round(avgPts * 10) / 10, totalYards: null, passingYards: null, rushingYards: null, turnovers: null },
                  pointsAllowed: Math.round(avgAllowed * 10) / 10,
                  gamesUsed: scores.length
                };
              }
            }
            console.log(`    ✓ Built recent stats from ${recentGames.length} game results for ${Object.keys(teamScores).length} teams`);
          }

          // Fetch raw news articles for teams that still lack news insights
          const teamsWithoutNews = teamNames.filter(t => !realData.newsInsights?.[t]);
          if (teamsWithoutNews.length > 0) {
            const threeDaysAgoISO = new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString();
            const { data: newsArticles } = await this.supabase
              .from('news_articles')
            ...
            if (newsArticles && newsArticles.length > 0) {
              if (!realData.newsInsights) realData.newsInsights = {};
              for (const team of teamsWithoutNews) {
                const mascot = team.split(' ').slice(-1)[0].toLowerCase();
                const relevant = newsArticles.filter(a =>
                  (a.title || '').toLowerCase().includes(mascot) ||
                  (a.summary || '').toLowerCase().includes(mascot)
                );
                if (relevant.length > 0) {
                  realData.newsInsights[team] = {
                    insights: relevant.map(a => `- ${a.title}`).join('\n'),
                    sentiment: 'neutral',
                    injuries: []
                  };
                }
              }
              console.log(`    ✓ Matched news articles to ${Object.keys(realData.newsInsights || {}).length} teams`);
            }
          }
        } catch (preAnalysisError) {
          console.error('  ⚠️ Error loading pre-analysis data:', preAnalysisError.message);
        }
      }
      
      // Inject ESPN intelligence from news_cache (standings, scores, injuries)
      if (this.supabase) {
        try {
          const sportNames = request.sports || [];
          const { data: espnIntel } = await this.supabase
            .from('news_cache')
            .select('sport, search_type, team_name, summary')
            .in('sport', sportNames)
            .in('search_type', ['injuries', 'standings', 'recent_results', 'upcoming_games'])
            .gt('last_updated', new Date(Date.now() - 12 * 60 * 60 * 1000).toISOString())
            .order('last_updated', { ascending: false });

          if (espnIntel && espnIntel.length > 0) {
            if (!realData.espnIntelligence) realData.espnIntelligence = {};
            for (const row of espnIntel) {
              const key = `${row.sport}_${row.search_type}`;
              if (!realData.espnIntelligence[key]) {
                realData.espnIntelligence[key] = row.summary;
              }
              // Also fill team injuries from ESPN if not already present
              if (row.search_type === 'injuries' && row.team_name && !realData.injuries[row.team_name]) {
                try {
                  const parsed = JSON.parse(row.summary);
                  if (Array.isArray(parsed) && parsed.length > 0) {
                    realData.injuries[row.team_name] = parsed.map(i => ({
                      player: i.player || 'Unknown',
                      position: '',
                      status: i.status || 'Listed',
                      injury: i.details || ''
                    }));
                  }
                } catch (e) {
                  // Summary might not be JSON, that's fine
                }
              }
            }
            console.log(`    ✓ Loaded ESPN intelligence: ${espnIntel.length} entries for ${sportNames.join(', ')}`);
          }
        } catch (espnErr) {
          console.warn('  ⚠️ ESPN intelligence load failed:', espnErr.message);
        }
      }

      // DEBUG: Log what data we're actually passing to AI
      console.log('\n📋 REAL DATA SUMMARY FOR AI:');
      console.log(`  Team Records: ${Object.keys(realData.teamRecords).length} teams`);
      console.log(`  Injuries: ${Object.keys(realData.injuries || {}).length} teams`);
      console.log(`  Team Stats: ${Object.keys(realData.teamStats || {}).length} teams`);
      console.log(`  Top Performers: ${Object.keys(realData.topPerformers || {}).length} teams`);
      console.log(`  News Articles: ${Object.values(realData.recentNews).flat().length} articles`);
      if (Object.keys(realData.teamRecords).length > 0) {
        console.log(`  Sample Records:`, Object.entries(realData.teamRecords).slice(0, 3).map(([team, data]) => `${team} ${data.record}`).join(', '));
      }
      if (Object.keys(realData.teamStats || {}).length > 0) {
        const sample = Object.entries(realData.teamStats)[0];
        console.log(`  Sample Team Stats: ${sample[0]} - ${sample[1].averages.points} PPG`);
      }
      if (Object.keys(realData.topPerformers || {}).length > 0) {
        const sample = Object.entries(realData.topPerformers)[0];
        if (sample[1].topPasser) {
          console.log(`  Sample Performer: ${sample[1].topPasser.name} - ${sample[1].topPasser.yards} yds`);
        }
      }
    ...
    try {
      const gameSnapshots = {};
      const teamStatsByTeam = realData.teamStats || {};
      const performersByTeam = realData.topPerformers || {};
      const newsByTeam = realData.newsInsights || {};
      
      const formatRecord = (teamName, rec) => {
        if (!teamName) return null;
        const wins = typeof rec?.wins === 'number' ? rec.wins : null;
        const losses = typeof rec?.losses === 'number' ? rec.losses : null;
        const ties = typeof rec?.ties === 'number' ? rec.ties : null;
        let recordStr = null;
        if (wins !== null && losses !== null) {
          recordStr = ties !== null ? `${wins}-${losses}-${ties}` : `${wins}-${losses}`;
        } else if (typeof rec?.record === 'string') {
          recordStr = rec.record;
        }
        const diff = typeof rec?.pointDifferential === 'number' ? rec.pointDifferential : null;
        const pf = rec?.pointsFor ?? rec?.points_for ?? null;
        const pa = rec?.pointsAgainst ?? rec?.points_against ?? null;
        const winPct = typeof rec?.winPercentage === 'number' ? rec.winPercentage : null;
        return {
          team: teamName,
          wins,
          losses,
          ...
          divisionRank: rec?.divisionRank || null
        };
      };
      
      const formatRecent = (stats) => {
        if (!stats || !stats.averages) return null;
        const a = stats.averages;
        return {
          last3Points: typeof a.points === 'number' ? a.points : null,
          last3TotalYards: typeof a.totalYards === 'number' ? a.totalYards : null,
          last3PassingYards: typeof a.passingYards === 'number' ? a.passingYards : null,
          last3RushingYards: typeof a.rushingYards === 'number' ? a.rushingYards : null,
          last3Turnovers: typeof a.turnovers === 'number' ? a.turnovers : null
        };
      };
      
      const simplifyInjuries = (list) => {
        return (list || []).map(inj => ({
          player: inj.player,
          position: inj.position || null,
          status: inj.status,
          note: inj.injury || inj.description || null
        }));
      };
      
      const simplifyPerformers = (perf) => {
        if (!perf) return {};
        const mapPerf = (p, fallbackPos) => {
          if (!p) return null;
          return {
            name: p.name,
            position: p.position || fallbackPos || null,
            yards: typeof p.yards === 'number' ? p.yards : null,
            touchdowns: typeof p.touchdowns === 'number' ? p.touchdowns : (p.touchdowns == null ? 0 : null),
            receptions: typeof p.receptions === 'number' ? p.receptions : null
          };
        };
        return {
          passer: mapPerf(perf.topPasser, 'QB'),
          rusher: mapPerf(perf.topRusher, 'RB'),
          receiver: mapPerf(perf.topReceiver, 'WR')
        };
      };
      
      const simplifyNews = (entry) => {
        if (!entry) return [];
        return [{
          summary: entry.insights || '',
          sentiment: entry.sentiment || null,
          injuriesMentioned: Array.isArray(entry.injuries) ? entry.injuries : []
        }];
      };
      ...
      for (const game of games || []) {
        ...
        if (gameSnapshots[key]) continue;
        
        const homeRecRaw = realData.teamRecords[homeTeam] || {};
        const awayRecRaw = realData.teamRecords[awayTeam] || {};
        const homeStats = teamStatsByTeam[homeTeam] || null;
        const awayStats = teamStatsByTeam[awayTeam] || null;
        const homePerfRaw = performersByTeam[homeTeam] || null;
        const awayPerfRaw = performersByTeam[awayTeam] || null;
        const homeNewsRaw = newsByTeam[homeTeam] || null;
        const awayNewsRaw = newsByTeam[awayTeam] || null;
        const homeInjuries = realData.injuries[homeTeam] || [];
        const awayInjuries = realData.injuries[awayTeam] || [];
        
        const homeRecord = formatRecord(homeTeam, homeRecRaw);
        const awayRecord = formatRecord(awayTeam, awayRecRaw);
        const homeRecent = formatRecent(homeStats);
        const awayRecent = formatRecent(awayStats);
        
        const sport = (game.sport_key || game.sport_title || game.sport || '').toString();
        const gameDate = game.commence_time || game.commenceTime || game.gameDate || null;
        
        gameSnapshots[key] = {
          gameId: game.id || game.game_id || key,
          sport,
          gameDate,
          homeTeam,
          awayTeam,
          ...
          injuries: {
            home: simplifyInjuries(homeInjuries),
            away: simplifyInjuries(awayInjuries)
          },
          topPerformers: {
            home: simplifyPerformers(homePerfRaw),
            away: simplifyPerformers(awayPerfRaw)
          },
          news: {
            home: simplifyNews(homeNewsRaw),
            away: simplifyNews(awayNewsRaw)
          }
        };
      }
      
      realData.gameSnapshots = gameSnapshots;
      console.log(`  📋 Built gameSnapshots for ${Object.keys(gameSnapshots).length} games`);
    } catch (snapshotError) {
      console.error('  ⚠️ Error building gameSnapshots:', snapshotError.message);
    }
