...
class MultiAgentCoordinator {
  ...
  /**
   * Verify player-team assignments using cached team data (no live provider call)
   */
   ...
   * - Would need a current season roster feed
  ...
  async gatherRealDataForAI(games, request) {
    ...
    try {
      ...
      // 2. News insights: the summarizer job was retired 2026-09-14 (owner,
      // build_queue 36). The game_analysis rows injected below carry the
      // narration context, which now reads ESPN news directly.
      realData.newsInsights = {};

      // 2.5 INJECT PRE-COMPUTED GAME ANALYSIS (from game_analysis table)
      // This fills gaps for NCAAB/NBA/NHL where cached team data is missing
      if (this.supabase) {
        try {
