  ...
  async enrichGamesWithPreAnalysis(games) {
    ...
    try {
      ...
      const { data: preAnalyses } = await this.supabase
        .from('game_analysis')
        .select('home_team, away_team, analysis_snippet, edge_score, recommended_pick, recommended_side, key_factors, spread, total, home_record, away_record, home_ranking, away_ranking, news_context, injury_context')
        .eq('stale', false)
