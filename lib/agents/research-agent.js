...
class EnhancedResearchAgent {
  constructor(fetcher, serperApiKey) {
    ...
    // Initialize NFL stats service
    try {
      this.nflStats = require('../services/nfl-stats');
      console.log('✅ NFL Stats service initialized');
    } catch (error) {
  ...
  async comprehensiveGameAnalysis(game, { fastMode = false, researchDepth = 'moderate' } = {}) {
    ...
    const isNFL = sport.toLowerCase().includes('nfl') || sport.toLowerCase().includes('americanfootball_nfl');
