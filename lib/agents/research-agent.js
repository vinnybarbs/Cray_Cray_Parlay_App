...
class EnhancedResearchAgent {
  ...
  async comprehensiveGameAnalysis(game, { fastMode = false, researchDepth = 'moderate', userSelectedNFL = false } = {}) {
    ...
    try {
      ...
      // Cached stats first for every supported sport, then RSS and Serper.
      let gameResearch;
      ...
      if (this.statsService && ['NFL', 'NBA', 'NCAAF', 'MLB', 'NHL', 'SOCCER', 'GOLF', 'TENNIS', 'UFC'].includes(sportKey)) {
        ...
        try {
          ...
          if (matchupContext) {
            ...
            sources.push({ title: 'Cached sports stats', link: 'supabase-cache' });
      ...
      if (!cachedStatsUsed) {
        // No cached stats: try RSS first, then Serper
  ...
  formatResearchForAI(games) {
    ...
    if (realStatsCount > 0) {
      lines.push(`\n**🎯 DATA SOURCE: ${realStatsCount} games with stored NFL stats**\n`);
    }
