...
class EnhancedResearchAgent {
  constructor(fetcher, serperApiKey, supabase = null) {
    ...
    // API-Sports (and its NFL stats service) is permanently decommissioned.
    // Downstream paths null-check this and fall back to cached/ESPN data.
    this.nflStats = null;
    ...
    try {
      ...
      console.log('✅ Sports Stats service initialized for cached data');
    } catch (error) {
      console.log('⚠️ Sports Stats service not available:', error.message);
      this.statsService = null;
    }

    // Initialize RSS research service (replaces Serper)
    try {
      const { RSSResearchService } = require('../services/rss-research');
      this.rssService = supabase ? new RSSResearchService(supabase) : null;
      if (this.rssService) {
        console.log('✅ RSS Research service initialized (replaces Serper)');
      }
    } catch (error) {
      console.log('⚠️ RSS Research service not available:', error.message);
      this.rssService = null;
    }
  }

  async deepResearch(games, { fastMode = false, numLegs = 3, riskLevel = 'Medium', selectedSports = [] } = {}) {
    // RSS research doesn't require API key, but log if no research available at all
    if (!this.serperApiKey && !this.rssService) {
      console.log('⚠️ No research services available (no Serper, no RSS)');
      return games.map(g => ({ ...g, research: null }));
    }
    
    if (!this.serperApiKey && this.rssService) {
      console.log('📰 Using RSS-only research (Serper disabled - cost savings!)');
    }
  ...
  async comprehensiveGameAnalysis(game, { fastMode = false, researchDepth = 'moderate', userSelectedNFL = false } = {}) {
    ...
    // Fix: Map sport_key to proper format for stats service
    let sportKey = game.sport_key || sport;
    ...
    try {
      ...
      // Try cached sports stats first for all supported sports
      let cachedStatsUsed = false;
      if (this.statsService && ['NFL', 'NBA', 'NCAAF', 'MLB', 'NHL', 'SOCCER', 'GOLF', 'TENNIS', 'UFC'].includes(sportKey)) {
        console.log(`    📊 Checking cached sports stats for ${sportKey}`);
        try {
          const matchupContext = await this.statsService.getMatchupContext(sportKey, game.home_team, game.away_team);
          if (matchupContext) {
            const cachedStatsSummary = this.formatCachedStatsForResearch(matchupContext, game);
            combinedResearch = cachedStatsSummary;
            sources.push({ title: 'Cached API Sports Stats', link: 'supabase-cache' });
            cachedStatsUsed = true;
            console.log(`    ✅ Cached sports stats retrieved for ${game.away_team} vs ${game.home_team}`);
          }
        } catch (statsError) {
          console.log(`    ⚠️ Cached stats error: ${statsError.message}`);
        }
      }

      // If cached stats failed, try NFL Stats API for NFL games
      if (!cachedStatsUsed && isNFL && this.nflStats) {
        ...
        try {
          ...
          if (statsAnalysis) {
            ...
            console.log(`    ⚠️ NFL stats unavailable, trying RSS research`);
            const rssResult = await this.tryRSSResearch(game.home_team, game.away_team, sportKey);
            if (rssResult.success) {
              combinedResearch = rssResult.research;
              sources.push(...rssResult.sources);
            } else {
          ...
          console.log(`    ⚠️ NFL stats error: ${error.message}, trying RSS research`);
          const rssResult = await this.tryRSSResearch(game.home_team, game.away_team, sportKey);
          if (rssResult.success) {
            combinedResearch = rssResult.research;
            sources.push(...rssResult.sources);
      ...
      } else if (!cachedStatsUsed) {
        // Non-NFL or no stats service: try RSS first, then Serper
        console.log(`    📰 Trying RSS research for ${game.away_team} vs ${game.home_team}`);
        const rssResult = await this.tryRSSResearch(game.home_team, game.away_team, sportKey);
        
        if (rssResult.success) {
          combinedResearch = rssResult.research;
          sources.push(...rssResult.sources);
          console.log(`    ✅ RSS research found ${rssResult.factCount} facts`);
        } else {
          console.log(`    ⚠️ No RSS data, falling back to Serper`);
          gameResearch = await this.performSerperFallback(game, currentYear, gameDate);
          const { summary: gameSummary, sources: gameSources } = this.synthesizeResearch(gameResearch, game);
          combinedResearch = gameSummary;
          sources.push(...gameSources);
  ...
  /**
   * Try RSS research (primary research source, replaces Serper)
   * Returns formatted research with PROMINENT timestamps so AI knows data freshness
   */
  async tryRSSResearch(homeTeam, awayTeam, sport) {
    if (!this.rssService) {
      return { success: false, research: '', sources: [], factCount: 0 };
    }

    try {
      const rssResearch = await this.rssService.getMatchupResearch(homeTeam, awayTeam, sport);
      
      if (!rssResearch || !rssResearch.facts || rssResearch.facts.length === 0) {
        return { success: false, research: '', sources: [], factCount: 0 };
      }

      // Format with CLEAR DATA FRESHNESS indicators
      const now = Date.now();
      ...
      // Build research with prominent timestamp warning
      let formattedResearch = `📰 NEWS (as of ${dateContext} - sports change daily!):\n`;
      
      rssResearch.facts.forEach((fact, idx) => {
        formattedResearch += `${idx + 1}. ${fact}\n`;
      });

      // Add recency warning for AI
      formattedResearch += `\n⚠️ NOTE: This news is from recent articles (last 48 hours). Player statuses and team situations can change rapidly in sports.`;

      const formattedSources = rssResearch.sources.map((source, idx) => ({
        idx: idx + 1,
        title: source.title,
        link: source.link || 'rss-feed',
      ...
      return {
        ...
        factCount: rssResearch.facts.length
      };

    } catch (error) {
      console.log(`    ⚠️ RSS research error: ${error.message}`);
      return { success: false, research: '', sources: [], factCount: 0 };
    }
  }
  ...
  /**
   * Format cached sports stats for research context
   */
  formatCachedStatsForResearch(matchupContext, game) {
    const { homeTeam, awayTeam, insights } = matchupContext;
    const sport = game.sport_key?.toUpperCase();
    
    let formattedStats = `${awayTeam.name} @ ${homeTeam.name} - Cached Sports Stats Analysis:\n\n`;
    
    // Team Statistics Comparison
    if (homeTeam.stats && awayTeam.stats) {
      formattedStats += `TEAM PERFORMANCE:\n`;
      ...
      if (sport === 'NFL') {
        ...
        if (homeStats.wins !== undefined && awayStats.wins !== undefined) {
          formattedStats += `• Records: ${homeTeam.name} (${homeStats.wins}-${homeStats.losses}), ${awayTeam.name} (${awayStats.wins}-${awayStats.losses})\n`;
        }
        
        if (homeStats.points_for && awayStats.points_for) {
          formattedStats += `• Scoring: ${homeTeam.name} avg ${homeStats.points_for}ppg, ${awayTeam.name} avg ${awayStats.points_for}ppg\n`;
        }
        
        if (homeStats.points_against && awayStats.points_against) {
          formattedStats += `• Defense: ${homeTeam.name} allows ${homeStats.points_against}ppg, ${awayTeam.name} allows ${awayStats.points_against}ppg\n`;
        }
        
        if (homeStats.total_yards && awayStats.total_yards) {
          formattedStats += `• Total Yards: ${homeTeam.name} ${homeStats.total_yards}ypg, ${awayTeam.name} ${awayStats.total_yards}ypg\n`;
        }
        
        if (homeStats.turnovers && awayStats.turnovers) {
          formattedStats += `• Turnovers: ${homeTeam.name} ${homeStats.turnovers}, ${awayTeam.name} ${awayStats.turnovers}\n`;
        }
      } else if (sport === 'NBA') {
        // NBA-specific stats formatting
        const homeStats = homeTeam.stats;
        const awayStats = awayTeam.stats;
        ...
        if (homeStats.points && awayStats.points) {
          formattedStats += `• Scoring: ${homeTeam.name} ${homeStats.points}ppg, ${awayTeam.name} ${awayStats.points}ppg\n`;
        }
        
        if (homeStats.field_goal_percentage && awayStats.field_goal_percentage) {
          formattedStats += `• FG%: ${homeTeam.name} ${homeStats.field_goal_percentage}%, ${awayTeam.name} ${awayStats.field_goal_percentage}%\n`;
        }
      }
    }
    
    // Key Players
    if (homeTeam.keyPlayers && homeTeam.keyPlayers.length > 0) {
      formattedStats += `\n${homeTeam.name} KEY PLAYERS:\n`;
      homeTeam.keyPlayers.slice(0, 3).forEach(player => {
        formattedStats += `• ${player.name} (${player.position})`;
        if (player.stats && sport === 'NFL') {
          const stats = player.stats;
          if (stats.passing_yards) formattedStats += ` - ${stats.passing_yards} pass yds, ${stats.passing_tds} pass TDs`;
          if (stats.rushing_yards) formattedStats += ` - ${stats.rushing_yards} rush yds, ${stats.rushing_tds} rush TDs`;
          if (stats.receiving_yards) formattedStats += ` - ${stats.receiving_yards} rec yds, ${stats.receiving_tds} rec TDs`;
        }
        formattedStats += '\n';
      });
    }
    
    if (awayTeam.keyPlayers && awayTeam.keyPlayers.length > 0) {
      formattedStats += `\n${awayTeam.name} KEY PLAYERS:\n`;
      awayTeam.keyPlayers.slice(0, 3).forEach(player => {
        formattedStats += `• ${player.name} (${player.position})`;
        if (player.stats && sport === 'NFL') {
          const stats = player.stats;
          if (stats.passing_yards) formattedStats += ` - ${stats.passing_yards} pass yds, ${stats.passing_tds} pass TDs`;
    ...
    if (insights && insights.length > 0) {
      ...
      insights.forEach(insight => {
        formattedStats += `• ${insight}\n`;
      });
    }
    
    formattedStats += `\n[Data Source: Cached API Sports Statistics]`;
