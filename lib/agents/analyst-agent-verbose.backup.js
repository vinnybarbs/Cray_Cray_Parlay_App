...
class ParlayAnalyst {
  ...
  if (oddsData && oddsData.length > 0) {
    // In fast mode, dramatically reduce items to speed up AI response
    const maxItems = fastMode ? Math.min(numLegs * 1.5, 6) : Math.min(Math.max(numLegs * 2, 6), 20);
    const items = oddsData.slice(0, Math.ceil(maxItems)).map((ev, idx) => {
      ...
      // Add research block with sources when available (skip in fast mode to reduce prompt size)
      if (!fastMode) {
        try {
        ...
        console.log('⚡ Fast mode: Skipping detailed research block to reduce prompt size');
      }
    } else {
  ...
  generateOpenAIPrompt({ sportsStr, betTypesStr, numLegs, riskLevel, today, dateRangeText, marketAvailabilityNote, oddsContext, attemptNumber, retryIssues = '' }) {
    ...
    return `${retry}Create a ${numLegs}-leg parlay.

RULES:
- EXACTLY ${numLegs} legs
- Bet types: ${betTypesStr}  
- Use EXACT dates/odds/lines from data
- No conflicts (no opposing bets)
- Risk: ${riskLevel}
...
\n\nMACHINE-READABLE BLOCK (REQUIRED):\nProvide a strict JSON object between the markers below. Use EXACTLY American odds strings like "+105" or "-120" (use "+100" for EV/EVEN/PK). Ensure legs length is EXACTLY ${numLegs}.\n\n===BEGIN_PARLAY_JSON===\n{\n  "parlay": {\n    "title": "[Title]",\n    "legs": [\n      {\n        "date": "MM/DD/YYYY",\n        "game": "Away @ Home",\n        "bet": "[Specific bet with line]",\n        "odds": "+100",\n        "confidence": 8,\n        "citations": [1,2]\n      }\n      // ... repeat until exactly ${numLegs} legs\n    ]\n  },\n  "lockParlay": {\n    "legs": [\n      // 2 legs, optional if not applicable\n    ]\n  }\n}\n===END_PARLAY_JSON===\n`.trim();
  ...
  async generateParlayWithAI(prompt, aiModel, fetcher, openaiKey, geminiKey) {
    ...
    // Log prompt size to debug latency
    const promptSize = prompt.length;
    const promptKb = (promptSize / 1024).toFixed(2);
    console.log(`📝 Prompt size: ${promptKb} KB (${promptSize} chars)`);
    ...
    if (aiModel === 'openai') {
      ...
      console.log(`✅ OpenAI responded in ${elapsedMs}ms (${(elapsedMs/1000).toFixed(1)}s)`);
      ...
      const elapsedMs = Date.now() - startTime;
      console.log(`✅ Gemini responded in ${elapsedMs}ms (${(elapsedMs/1000).toFixed(1)}s)`);
    }
    
    // Log response size
    const responseSize = content.length;
    const responseKb = (responseSize / 1024).toFixed(2);
    console.log(`📥 Response size: ${responseKb} KB (${responseSize} chars)`);
