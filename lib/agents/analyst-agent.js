...
**PRE-COMPUTED AI GAME ANALYSIS** (Data-backed edge detection by GPT-4o-mini):
${(realData.gameAnalysis || [])
  .filter(a => activeTeams.has(a.home_team) || activeTeams.has(a.away_team))
  .map(a => {
    const parts = [`${a.away_team} @ ${a.home_team} (${a.sport})`];
    if (a.home_record || a.away_record) parts.push(`  Records: ${a.away_team} ${a.away_record || '?'} vs ${a.home_team} ${a.home_record || '?'}`);
    if (a.home_ranking) parts.push(`  ${a.home_team} ranked #${a.home_ranking}`);
    if (a.away_ranking) parts.push(`  ${a.away_team} ranked #${a.away_ranking}`);
    if (a.spread) parts.push(`  Spread: ${a.home_team} ${a.spread}`);
    if (a.total) parts.push(`  O/U: ${a.total}`);
    if (a.analysis_snippet) parts.push(`  Analysis: ${a.analysis_snippet}`);
    if (a.key_factors && a.key_factors.length) parts.push(`  Key Factors: ${a.key_factors.join(', ')}`);
    if (a.recommended_pick) parts.push(`  ➜ Recommended: ${a.recommended_pick} (edge: ${a.edge_score}/10, side: ${a.recommended_side})`);
    return parts.join('\n');
