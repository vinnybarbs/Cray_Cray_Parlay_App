...
4. **For PLAYER PROPS:** Use `get_player_stats()` function call to retrieve concrete stats (avg receptions, yards, TDs). NEVER use hedging language like "data not available in cache", "lighter lean", or "should be treated as" in reasoning. If you cannot retrieve player stats via function call, DO NOT SELECT that player prop - skip it entirely and choose a different bet with verified data.
