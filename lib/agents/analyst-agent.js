  ...
  async selectBestPicks({ picks, numSuggestions, riskLevel, betTypes, apiKey, realData = null, historicalLessons = [] }) {
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o',
        messages: [
          { role: 'system', content: `You are a sports betting analyst. Write specific, data-driven reasoning using the VERIFIED DATA section.

REQUIREMENTS:
- Use actual player names and stats from the data
- Reference recent trends (e.g., "averaging X over last 3 games")
- Mention injuries by player name if relevant
- NO generic phrases like "favorable environment" - use specific numbers
- Example good reasoning: "Mahomes averaging 285 yds/game in last 3, facing defense allowing 28 PPG"
- Example bad reasoning: "Favorable matchup in good game environment"
...
**News Insights** (Pre-analyzed betting-relevant facts):
