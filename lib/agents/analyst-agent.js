...
`.trim();
   ...
   */
  async selectBestPicks({ picks, numSuggestions, riskLevel, betTypes, apiKey, realData = null }) {
    console.log(`\n🤖 AI analyzing ${picks.length} picks to select best ${numSuggestions}...`);
    
    // Build prompt for AI to rank picks WITH REAL DATA
    const prompt = this.buildPickSelectionPrompt(picks, numSuggestions, riskLevel, betTypes, realData);
...
Your analysis should be insightful and varied, but 100% grounded in provided facts. Return ONLY valid JSON.` },
   ...
   */
  buildPickSelectionPrompt(picks, numSuggestions, riskLevel, betTypes, realData = null) {
...
**Team Records (W-L)**:
${Object.entries(realData.teamRecords || {}).map(([team, data]) => `- ${team}: ${data.record}`).join('\n') || 'No records available'}
...
`;
    ...
    return `${realDataSection}You are an ANALYTICAL EDGE DETECTOR, not a pick justifier. Your job is to find genuine market inefficiencies and analytical advantages.
...
- BE SPECIFIC but ONLY with provided data: Reference team W-L records, recent news headlines, and research summaries
- NEVER make up stats: If ATS records, injury details, or recent game stats are not in the data above, DO NOT mention them
- FOCUS ON: Odds value, matchup context from W-L records, and general team strength
  ...
  "picks": [
    {
      ...
      "reasoning": "2-3 sentences using ONLY provided data. Reference team W-L records, news headlines, odds value, and research intel. DO NOT make up stats. Example: 'Steelers (9-1) face Bills (7-5) at home. Recent news shows Bills struggling on road. Steelers getting +160 as home underdog offers value given their strong record.'",
