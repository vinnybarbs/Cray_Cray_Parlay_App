...
5. **PLAYER PROPS MUST NAME SPECIFIC PLAYERS**:
   - NEVER use generic prop descriptions like "Anytime Touchdown", "Passing Yards", "Receptions"
   - ALWAYS specify the exact player: "Jahmyr Gibbs Anytime Touchdown", "Jared Goff Over 245.5 Passing Yards"
   - Format: "[Player Name] [Prop Type] [Line if applicable]"
   - Examples: ✅ "Christian McCaffrey Anytime Touchdown +110" ❌ "Anytime Touchdown +110"
   - Examples: ✅ "Jayden Daniels Over 225.5 Passing Yards -110" ❌ "Over 225.5 Passing Yards -110"

6. **PLAYER PROPS MUST MATCH GAME TEAMS**:
   - For player props, the player MUST play for the Away team OR the Home team listed in that game
   - Example: For "Patriots @ Saints" game, only use Patriots or Saints players
   - DO NOT use players from other teams in that game's prop bets
6. **NEVER INVENT ODDS, SPREADS, OR FAVORITES**:
...
`.trim();
   ...
   */
  async selectBestPicks({ picks, numSuggestions, riskLevel, betTypes, apiKey }) {
    console.log(`\n🤖 AI analyzing ${picks.length} picks to select best ${numSuggestions}...`);
    
    // Build prompt for AI to rank picks
    const prompt = this.buildPickSelectionPrompt(picks, numSuggestions, riskLevel, betTypes);
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'You are an expert sports betting analyst with access to comprehensive team/player statistics and real-time intelligence. Use cached intelligence (🧠 Intel) and stats data (📊 Stats) for informed decisions. Reference injury reports, expert picks, and betting trends in your analysis. Analyze picks and return ONLY valid JSON.' },
          { role: 'user', content: prompt }
        ],
        temperature: 0.3,
        max_tokens: 4000,
    ...
    const selectedPicks = (result.picks || []).map(pick => ({
      ...
      researchSummary: pick.researchSummary
    }));
  ...
  /**
   * Build prompt for AI to select best picks
   */
  buildPickSelectionPrompt(picks, numSuggestions, riskLevel, betTypes) {
    const riskGuidelines = {
      'Low': 'Favor favorites (negative odds), avoid heavy underdogs, prefer moneylines over spreads',
      'Medium': 'Balance favorites and underdogs, mix bet types, target value',
      'High': 'Consider underdogs, player props, and higher-risk/higher-reward bets'
    };
    
    // Sample picks to avoid token limits (max 100 picks)
    const sampledPicks = picks.length > 100 ? 
      picks.sort(() => 0.5 - Math.random()).slice(0, 100) : 
      picks;
    
    const picksContext = sampledPicks.map((p, idx) => {
      return `${idx + 1}. ID: ${p.id}
   Game: ${p.awayTeam} @ ${p.homeTeam} (${new Date(p.gameDate).toLocaleDateString()})
   Bet: ${p.betType} - ${p.pick} ${p.point ? `(${p.point})` : ''} @ ${p.odds}
   Spread Context: ${p.spread || 'N/A'}
   Research: ${p.research.substring(0, 200)}...`;
    }).join('\n\n');
    
    return `You are selecting the ${numSuggestions} best individual betting picks from ${sampledPicks.length} options.

**Risk Level**: ${riskLevel}
**Guidelines**: ${riskGuidelines[riskLevel]}
**Bet Types Requested**: ${betTypes.join(', ')}

**CRITICAL RULES**:
1. Each pick must be INDEPENDENT - no same-game correlations
2. Diversify across games - max 2 picks from same game
3. Always include spread context even for moneyline bets
4. Confidence score 1-10 based on research and value
5. Reasoning must reference specific research insights
  ...
  "picks": [
    {
      ...
      "pick": "Team/Player Name or Over/Under",
      "odds": "American odds",
      "spread": "Spread value for context (e.g., -10.5) or null",
      "confidence": 8,
      "reasoning": "Why this is a strong pick based on research",
      "researchSummary": "Key insights from research"
    }
  ]
}

Select exactly ${numSuggestions} picks. Return ONLY valid JSON, no markdown.`;
  }
}
