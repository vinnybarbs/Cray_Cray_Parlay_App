  ...
  async selectBestPicks({ picks, numSuggestions, riskLevel, betTypes, apiKey, realData = null, historicalLessons = [] }) {
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o',
        messages: [
          { role: 'system', content: `You are an expert sports betting analyst with access to real-time API-Sports data. You MUST follow these STRICT rules:

1. **USE THE VERIFIED DATA**: You have injury reports, team records, recent performance stats, and top performers from API-Sports
2. **BE SPECIFIC**: Reference actual player names, stats, and trends (e.g., "Patrick Mahomes averaging 285 yds/game in last 3")
3. **NO GENERIC REASONING**: Instead of "favorable game environment", say "Chiefs averaging 28 PPG vs Raiders allowing 24"
4. **CITE INJURIES**: If a key player is out, NAME THEM and explain impact
5. **USE PERFORMER DATA**: Reference actual QB/RB/WR stats when picking player props
6. **DO NOT INVENT DATA**: Only use what's in the VERIFIED DATA section

Your reasoning MUST include:
- Specific player names and their actual stats
- Recent performance trends (e.g., "last 3 games averaging X")
- Injury impacts (name the player and their role)
- Matchup analysis using real offensive/defensive stats

Return ONLY valid JSON with detailed, data-driven reasoning.` },
          { role: 'user', content: prompt }
        ],
        temperature: 0.3,
        max_tokens: 4000,
...
**Recent Team Performance** (Last 3 Games - API-Sports):
${Object.entries(realData.teamStats || {}).map(([team, stats]) => {
  return `${team}:
  • Averaging ${stats.averages.points} PPG, ${stats.averages.totalYards} total yards
  • Passing: ${stats.averages.passingYards} yds/game | Rushing: ${stats.averages.rushingYards} yds/game
  • Turnovers: ${stats.averages.turnovers} per game`;
}).join('\n') || 'No recent stats available'}

**Top Performers** (Season Leaders - API-Sports):
${Object.entries(realData.topPerformers || {}).map(([team, performers]) => {
  const parts = [];
  if (performers.topPasser) {
    parts.push(`  QB: ${performers.topPasser.name} (${performers.topPasser.yards} yds, ${performers.topPasser.touchdowns} TDs)`);
  }
  if (performers.topRusher) {
    parts.push(`  RB: ${performers.topRusher.name} (${performers.topRusher.yards} yds, ${performers.topRusher.touchdowns} TDs)`);
  }
  if (performers.topReceiver) {
    parts.push(`  WR: ${performers.topReceiver.name} (${performers.topReceiver.receptions} rec, ${performers.topReceiver.yards} yds, ${performers.topReceiver.touchdowns} TDs)`);
  }
