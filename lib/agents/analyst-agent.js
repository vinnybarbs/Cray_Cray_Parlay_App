...
`.trim();
  ...
  /**
   * Build analytical edge-detection prompt for AI
   */
  buildPickSelectionPrompt(picks, numSuggestions, riskLevel, betTypes) {
    const edgeDetectionFramework = {
      'Low': 'Find CLEAR MISMATCHES: Heavy favorites in bad spots, public overreactions, key injury advantages',
      'Medium': 'Find VALUE DISCREPANCIES: Line movement vs news, market overreactions, situational advantages', 
      'High': 'Find HIDDEN EDGES: Contrarian angles, advanced metrics, overlooked matchups'
    };
   ...
   ALL PICKS: ${group.picks.map(p => `${p.id}:${p.betType}-${p.pick}@${p.odds}`).join(', ')}`;
    ...
    return `You are an ANALYTICAL EDGE DETECTOR, not a pick justifier. Your job is to find genuine market inefficiencies and analytical advantages.
...
- If research shows no clear advantage, SKIP that game entirely
- Confidence must reflect genuine analytical conviction, not hope
- Look for CONTRADICTIONS between research and betting lines
- BE SPECIFIC: Include actual stats, injuries, trends (e.g., "Bills allow 4.8 YPC vs run-heavy Titans" not "matchup favors")
- QUANTIFY when possible: "Chiefs 7-2 ATS as favorites" not "Chiefs have good record"
- REFERENCE the research: Use specific intel from the research summary provided

**Games & Market Analysis**:
${analyticalContext}

**Your Task**: Analyze each game's research vs betting lines. Find GENUINE analytical edges where the line doesn't match the reality of the matchup. Ignore games where you can't find a clear edge.

**Return ONLY games with genuine edges in this format**:
{
  ...
  "picks": [
    {
      ...
      "reasoning": "2-3 sentences with SPECIFIC data from research: actual stats, injury details, trends with numbers. Example: 'Titans rushing attack (4th in YPC at 5.1) faces Bills defense allowing 4.8 YPC (22nd). Bills missing starting LB weakens run defense. Titans 6-2 ATS as underdogs this season.'",
      "contrary_evidence": "1-2 sentences: What evidence contradicts this pick? Be intellectually honest."
    }
  ]
}

Find maximum ${numSuggestions} picks, but ONLY include games with genuine analytical edges. Return ONLY valid JSON.`;
  }
}
