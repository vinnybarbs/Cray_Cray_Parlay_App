    ...
    const items = oddsData.slice(0, maxItems).map((ev, idx) => {
        ...
        if (ev.statsContext && ev.statsContext.dataSource === 'cached_sports_stats') {
          ...
          if (homeStats && awayStats) {
            ...
            if (ev.sport_key?.toUpperCase() === 'NFL') {
        ...
        return `${idx + 1}. DATE: ${gameDate} - ${teams}\n   ${marketsSummary}${researchSummary}${statsContext}`;
      });
  ...
  async generateParlayWithAI(prompt, fetcher, openaiKey) {
    ...
    const response = await fetcher('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'You are an expert sports betting analyst with access to comprehensive team/player statistics. Prioritize cached stats data (📊 Stats) when making decisions. Follow all rules exactly.' },
  ...
  async selectBestPicks({ picks, numSuggestions, riskLevel, betTypes, apiKey }) {
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'You are an expert sports betting analyst with access to comprehensive team/player statistics. Use cached stats data (📊 Stats) for informed decisions. Analyze picks and return ONLY valid JSON.' },
          { role: 'user', content: prompt }
        ],
        temperature: 0.3,
        max_tokens: 4000,
