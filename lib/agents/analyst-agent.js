...
const aiInstructions = require('../services/ai-instructions');
const { getClient: getClaude, MODELS } = require('../services/claude');

// Single source of truth for which model generates picks. Exported so
// suggest-picks can report it to the client, which stamps it on lock records
// (parlays.ai_model). The ledger must record what actually ran, not a
// client-side constant.
const ANALYST_MODEL = MODELS.NARRATION;
...
class ParlayAnalyst {
  constructor() {
    ...
    // Tool schemas (JSON Schema parameters, converted to Anthropic tool shape at call time)
  ...
  if (oddsData && oddsData.length > 0) {
    // Show more games than requested legs so every leg can land on a
    // different game. Same-game stacking is a thin-slate fallback only.
    const maxItems = Math.min(Math.max(numLegs + 2, 8), 12);
    const items = oddsData.slice(0, maxItems).map((ev, idx) => {
        ...
        if (ev.statsContext && ev.statsContext.dataSource === 'cached_sports_stats') {
          ...
          if (homeStats && awayStats) {
            ...
            // Map sport_key slug to friendly name
            const slugMap = {
            ...
            const friendlySport = slugMap[sport] || sport;

            if (friendlySport === 'NFL' || friendlySport === 'NCAAF') {
            ...
            } else if (friendlySport === 'NBA' || friendlySport === 'NCAAB') {
              if (homeStats.wins && awayStats.wins) {
                statsSummary.push(`Records: ${ev.statsContext.homeTeam.name} ${homeStats.wins}-${homeStats.losses}, ${ev.statsContext.awayTeam.name} ${awayStats.wins}-${awayStats.losses}`);
              }
    ...
    return this.generateAnalystPrompt({ sportsStr, betTypesStr, numLegs, riskLevel, oddsContext, attemptNumber, retryIssues });
  }

  generateAnalystPrompt({ sportsStr, betTypesStr, numLegs, riskLevel, oddsContext, attemptNumber, retryIssues }) {
    ...
    if (riskLevel === 'Low') {
      confidenceRule = '- CONFIDENCE REQUIREMENT: ALL picks MUST be 8-9/10 confidence. EASY MONEY MODE = moneyline favorites ONLY. Goal: WIN every pick. Lower payouts are fine, we want winners.';
      confidenceExample = '8/10 or 9/10';
      combinedOddsTarget = '- TARGET ODDS PER PICK: -150 to -500 range (strong favorites). Combined parlay odds +200 to +500.';
      oddsPolicyRule = '- ODDS POLICY: MONEYLINE FAVORITES ONLY. No spreads, no totals, no props. Pick the team most likely to WIN outright. Accept American odds from -150 down to -1000. NEVER suggest underdogs or plus-money picks in this mode.';
      confidenceCalibrationRule = '- CONFIDENCE-ODDS CALIBRATION: -150 to -200 ≈ 7-8/10. -201 to -400 ≈ 8-9/10. -401+ ≈ 9/10. Every pick must be a team you genuinely believe will WIN the game, backed by record, recent form, and matchup data.';
...
PARLAY STRUCTURE (FIXED, NON-NEGOTIABLE):
- One leg per game. Every leg MUST come from a DIFFERENT game in the data below.
- Same-game legs are a fallback ONLY when the data has fewer games than ${numLegs}. In that case cap it at 2 legs per game, using different bet types, never correlated or opposing bets.
- NEVER build the parlay from a single game when multiple games are available.
- When the user selected multiple bet types, vary bet types across the legs.

WRITING STYLE:
- Plain punctuation only. Never use em dashes, en dashes, or semicolons anywhere in your output. Use periods and commas.
${confidenceRule}
${combinedOddsTarget}
${oddsPolicyRule}
...
`.trim();
  ...
  // fetcher/apiKey params kept for call-site compatibility (coordinator
  // still passes them); the Anthropic client reads its own env key.
  async generateParlayWithAI(prompt) {
    let content = '';
    ...
    console.log(`🚀 Calling Claude (${ANALYST_MODEL})...`);
    const data = await claude.messages.create({
      model: ANALYST_MODEL,
      max_tokens: 1400,
      thinking: { type: 'disabled' },
      system: 'You are an expert sports betting analyst with access to comprehensive team/player statistics and real-time intelligence. Prioritize cached intelligence (🧠 Intel) and stats data (📊 Stats) when making decisions. Use injury reports, analyst picks, and betting trends to provide compelling reasoning. Follow all rules exactly.',
      messages: [{ role: 'user', content: prompt }],
    });
    content = data.content.filter(b => b.type === 'text').map(b => b.text).join('\n') || '';

    const elapsedMs = Date.now() - startTime;
    console.log(`✅ Claude: ${elapsedMs}ms (${(elapsedMs/1000).toFixed(1)}s)`);
  ...
  async selectBestPicks({ picks, numSuggestions, riskLevel, betTypes, apiKey, realData = null, historicalLessons = [], supabase = null }) {
    ...
    const playbook = await aiInstructions.getForMode(riskLevel);
    if (playbook) {
      console.log(`   📖 Loaded AI playbook (${playbook.length} chars)`);
    }
    ...
    // System prompt for tool-use pick selection
    const systemPrompt = `${playbook ? playbook + '\n\n---\n\n' : ''}You are a data-driven sports betting analyst. Your picks are backed ONLY by verified data provided to you. You NEVER invent statistics, records, player names, or trends.

ABSOLUTE RULES. VIOLATION MEANS YOUR OUTPUT IS WORTHLESS:
1. ONLY cite statistics, records, and facts from the VERIFIED DATA section below
2. NEVER invent W-L records, ATS records, streak data, or scoring averages
3. NEVER fabricate player names. Only mention players listed in injury reports or top performers data
4. If you don't have data for a claim, say "based on the line movement" or "market implies". Do NOT make up stats
5. Use the ACTUAL game dates from the picks data. Do NOT change them
6. Use the ACTUAL team names from the picks data. Do NOT rename teams
7. WRITING STYLE: Plain punctuation only. Never use em dashes, en dashes, or semicolons in your output. Use periods and commas.

YOUR APPROACH:
- Analyze matchups using ONLY the team records, injuries, standings, and news provided
- Reference specific numbers from VERIFIED DATA: "Iowa State (27-7, #6 ranking)" not "Iowa State has been dominant"
- When data is thin, focus on odds value and line analysis instead of fabricating context
- Your goal: help bettors WIN with honest, data-backed reasoning

${aiFunctions ? `
MANDATORY RESEARCH PROTOCOL:
You MUST research EVERY pick before suggesting it. DO NOT suggest picks you haven't researched.

REQUIRED FUNCTION CALL GUIDELINES:
1. **Team bets (Spread/Moneyline/Total):** Call get_team_stats() for teams
   - ONLY call for major professional teams: NFL, NBA, MLB, NHL
   - DO NOT call for college teams (NCAAF, NCAAB) - they won't have stats data
   - If get_team_stats returns "N/A" or placeholder data, DO NOT retry for that team
   - Example for "Jaguars @ Titans": 
     → get_team_stats("Jacksonville Jaguars") 
     → get_team_stats("Tennessee Titans")
   - Example for "Duke @ Virginia": DO NOT call get_team_stats (college teams)
...
PROCESS:
Step 1: Research games - call get_team_stats for NFL/NBA/MLB/NHL teams ONLY
Step 2: For college teams (NCAAF/NCAAB), skip stats calls and use provided data
Step 3: If stats return "N/A", use team records from the provided data instead
Step 4: Analyze the actual numbers (wins, PPG, points allowed, differential)
Step 5: Write reasoning with those specific numbers
Step 6: Return JSON with data-backed reasoning
` : `
DATA GUIDELINES:
...
REASONING FORMAT (MANDATORY, every reasoning MUST include):
1. Both teams' W-L records from VERIFIED DATA (e.g. "Iowa (21-12) vs Clemson (24-10)")
2. At least ONE specific fact: injury name, ranking, recent score, or news headline from the data
3. WHY the line offers value, not just "odds indicate" or "favored to win"
Example: "Iowa State (27-7, #6) hosts Tennessee State (23-9). ISU's defense held opponents to 57 pts last game. TSU allowed 88 and 96 pts in recent losses. The 24.5 spread is steep but ISU's blowout tendencies at home support it."
BAD: "Iowa State is favored and the odds reflect their superiority." (ZERO specific data = REJECTED)

REASONING STYLE:
✅ "Iowa State (27-7, #6) at home vs Tennessee State (23-9). Iowa State's 35-point average margin in last two wins and dominant home record justify the spread."
✅ "Clemson (24-10) vs Iowa (21-12). Line at -1.5 is tight. Clemson injury report shows key player OUT which shifts value to Iowa."
❌ "Iowa has been dominant recently, winning 7 of their last 8" (FABRICATED, their actual record shows otherwise)
❌ "John Doe Over 25.5 Points" (FABRICATED player name, not from any data source)
...
` : 'Return valid JSON only.'}`;
    ...
    // Tool-use loop. The model can query the database dynamically.
    const claude = getClaude();
    if (!claude) throw new Error('Server missing ANTHROPIC_API_KEY');

    // Anthropic tool shape: {name, description, input_schema}
    const claudeTools = aiFunctions
      ? this.functionSchemas.map(schema => ({
          name: schema.name,
    ...
    let maxIterations = 5; // Prevent loops (was 10, caused timeouts for college sports)
    let iteration = 0;
    let finalContent = null;

    const callModel = async () => claude.messages.create({
      model: ANALYST_MODEL,
      max_tokens: 4000,
      thinking: { type: 'disabled' },
      system: systemPrompt,
    ...
    while (iteration < maxIterations) {
      ...
      if (toolUses.length === 0) {
        ...
        finalContent = textOut;
        console.log(`✅ AI finished after ${iteration} iteration(s)`);

        // If response doesn't look like JSON, ask the model to format it properly
        if (finalContent && !finalContent.trim().startsWith('{')) {
          ...
          try {
            const jsonData = await callModel();
            const jsonText = jsonData.content.filter(b => b.type === 'text').map(b => b.text).join('\n');
            if (jsonText) {
              finalContent = jsonText;
              console.log('✅ Reformatted as JSON');
            }
          } catch (e) {
            console.warn(`JSON reformat call failed: ${e.message}`);
          }
        }
        break;
      }
      ...
      // Execute tool calls
      console.log(`   🔧 AI requested ${toolUses.length} tool calls`);

      // Append the assistant turn (full content, preserving tool_use blocks)
      messages.push({ role: 'assistant', content: data.content });

      // Execute each tool and collect results into ONE user message
      const toolResults = [];
    ...
    const selectedPicks = (result.picks || []).map(pick => {
      ...
      // Prefer basePick.research (enriched with pre-analysis) over AI echo
      let researchSummary = basePick.research || pick.researchSummary || '';
      ...
      // Strip unhelpful default placeholder
      if (typeof researchSummary === 'string' && researchSummary.trim() === 'No research data available') {
        researchSummary = '';
      }

      return {
        id: pick.id,
        gameDate: basePick.gameDate || pick.gameDate, // Prefer original commence_time over AI's date-only string
        commenceTime: basePick.gameDate || pick.gameDate, // Full ISO timestamp for frontend display
        sport: pick.sport,
        homeTeam: pick.homeTeam,
        awayTeam: pick.awayTeam,
        betType: pick.betType,
  ...
  buildPickSelectionPrompt(picks, numSuggestions, riskLevel, betTypes, realData = null, historicalLessons = []) {
    const edgeDetectionFramework = {
      'Low': 'EASY MONEY: Find moneyline favorites most likely to WIN outright. Focus on teams with best records, home advantages, and dominant recent form.',
    ...
    const analyticalContext = Array.from(gameGroups.values()).map((group, idx) => {
      ...
      // Build per-game INTELLIGENCE BRIEF, everything the AI needs for THIS game, right here
      const gameBrief = [];
      const awayTeam = group.game.split(' @ ')[0];
      const homeTeam = group.game.split(' @ ')[1];

      if (realData) {
        // Records
        const awayRec = realData.teamRecords?.[awayTeam];
        const homeRec = realData.teamRecords?.[homeTeam];
        if (awayRec || homeRec) {
          const awayStr = awayRec ? `${awayTeam} ${awayRec.record || `${awayRec.wins}-${awayRec.losses}`}${awayRec.ranking ? ` (#${awayRec.ranking})` : ''}${awayRec.streak ? ` ${awayRec.streak}` : ''}` : awayTeam;
          const homeStr = homeRec ? `${homeTeam} ${homeRec.record || `${homeRec.wins}-${homeRec.losses}`}${homeRec.ranking ? ` (#${homeRec.ranking})` : ''}${homeRec.streak ? ` ${homeRec.streak}` : ''}` : homeTeam;
          gameBrief.push(`RECORDS: ${awayStr} vs ${homeStr}`);
        }

        // Injuries for both teams
        for (const team of [awayTeam, homeTeam]) {
          const injuries = realData.injuries?.[team];
          if (injuries && injuries.length > 0) {
            const injList = injuries.slice(0, 5).map(i =>
              `${i.player} (${i.status}${i.injury ? ': ' + i.injury.substring(0, 40) : ''})`
            ).join(', ');
            gameBrief.push(`INJURIES ${team}: ${injList}`);
          }
        }

        // Recent stats (PPG)
        for (const team of [awayTeam, homeTeam]) {
          const stats = realData.teamStats?.[team];
          if (stats?.averages?.points) {
            const allowed = stats.pointsAllowed ? `, allowing ${stats.pointsAllowed} PPG` : '';
            gameBrief.push(`STATS ${team}: ${stats.averages.points} PPG${allowed} (last ${stats.gamesUsed || 3} games)`);
          }
        }

        // News insights
        for (const team of [awayTeam, homeTeam]) {
          const news = realData.newsInsights?.[team];
          if (news?.insights) {
            gameBrief.push(`NEWS ${team}: ${news.insights.substring(0, 200)}`);
          }
        }

        // Pre-computed game analysis
        const analysis = (realData.gameAnalysis || []).find(a =>
          (a.home_team === homeTeam && a.away_team === awayTeam) ||
          (a.home_team === awayTeam && a.away_team === homeTeam)
        );
        if (analysis) {
          if (analysis.analysis_snippet) gameBrief.push(`AI ANALYSIS: ${analysis.analysis_snippet.substring(0, 300)}`);
          if (analysis.key_factors?.length) gameBrief.push(`KEY FACTORS: ${analysis.key_factors.join(', ')}`);
          if (analysis.recommended_pick) gameBrief.push(`PRE-ANALYSIS PICK: ${analysis.recommended_pick} (edge: ${analysis.edge_score}/10)`);
        }
      }

      // Include research (ESPN intelligence) if not already covered
      const researchText = group.research || '';
      const researchSnippet = researchText.length > 0 ? researchText.substring(0, 400) : '';

      // Render picks menu with math-edge annotations when available so the
      // LLM sees per-side EV next to each option. Picks with positive
      // signedEdge get a ✅ marker; the math-recommended side gets ⭐.
      const picksMenu = group.picks.map(p => {
        const base = `${p.id}:${p.betType}-${p.pick}@${p.odds}${p.point ? '(' + p.point + ')' : ''}`;
        if (p.signedEdge == null) return base;
        const sign = p.signedEdge >= 0 ? '+' : '';
        const marker = p.isMathPick ? ' ⭐MATH PICK' : (p.signedEdge >= 0.02 ? ' ✅' : (p.signedEdge < -0.02 ? ' ❌' : ''));
        return `${base} [edge ${sign}${(p.signedEdge * 100).toFixed(1)}pp${marker}]`;
      }).join(', ');

      return `
━━━ GAME ${idx + 1}: ${group.game} (${new Date(group.date).toLocaleDateString()}) ━━━
${gameBrief.length > 0 ? gameBrief.join('\n') : 'NO VERIFIED DATA. Use only odds/line analysis'}
${researchSnippet ? `INTEL: ${researchSnippet}` : ''}
MARKETS: ${marketAnalysis.join(' | ')}
PICKS TO CHOOSE FROM: ${picksMenu}`;
    }).join('\n\n');
    ...
    if (realData) {
      ...
      realDataSection = `
**=== VERIFIED DATA (USE THIS, DO NOT INVENT STATS) ===**

**GAME BRIEFS** (Pre-analyzed by AI. YOUR PRIMARY SOURCE for records, edges, and key factors):
${(realData.gameAnalysis || [])
  .filter(a => activeTeams.has(a.home_team) || activeTeams.has(a.away_team))
  .map(a => {
    const lines = ['>> ' + a.away_team + ' @ ' + a.home_team + ' (' + a.sport + ')'];
    if (a.home_record || a.away_record) lines.push('  Records: ' + a.away_team + ' ' + (a.away_record || '?') + ' vs ' + a.home_team + ' ' + (a.home_record || '?'));
    if (a.home_ranking) lines.push('  ' + a.home_team + ' ranked #' + a.home_ranking);
    if (a.away_ranking) lines.push('  ' + a.away_team + ' ranked #' + a.away_ranking);
    if (a.spread) lines.push('  Spread: ' + a.home_team + ' ' + a.spread);
    if (a.total) lines.push('  O/U: ' + a.total);
    if (a.analysis_snippet) lines.push('  ANALYSIS: ' + a.analysis_snippet);
    if (a.key_factors && a.key_factors.length) lines.push('  Key Factors: ' + a.key_factors.join(', '));
    if (a.recommended_pick) lines.push('  -> Recommended: ' + a.recommended_pick + ' (edge: ' + a.edge_score + '/10)');
    return lines.join('\n');
  }).join('\n\n') || 'No game briefs available. Use injury/standings data below'}
...
${Object.entries(realData.teamStats || {})
  ...
  .map(([team, stats]) => {
    ...
    // Adapt formatting based on available data (football vs basketball)
    const hasFBStats = a.totalYards != null || a.passingYards != null;
    if (hasFBStats) {
    ...
    // Basketball / generic: just PPG + points allowed
    const ppg = a.points ?? 'N/A';
    const allowed = stats.pointsAllowed != null ? `, allowing ${stats.pointsAllowed} PPG` : '';
    const gamesNote = stats.gamesUsed ? ` (${stats.gamesUsed} games)` : '';
    return `${team}: Averaging ${ppg} PPG${allowed}${gamesNote}`;
...
**ESPN LIVE INTELLIGENCE** (Scores, Standings, Upcoming):
${Object.entries(realData.espnIntelligence || {}).map(([key, summary]) => {
  const [sport, type] = key.split('_');
  return `[${sport} ${type}]: ${summary.substring(0, 600)}`;
}).join('\n\n') || 'No ESPN intelligence available'}
...
**MATH-GRADED PICKS (READ THIS):**
Each pick in "PICKS TO CHOOSE FROM" shows its model edge in [edge +X.Xpp]:
  ⭐ MATH PICK = the side our edge calculator chose (highest +EV across all markets for that game)
  ✅          = positive edge ≥ +2pp (worth considering)
  ❌          = negative edge ≤ -2pp (model says this is a TRAP, don't pick it)
  no marker   = noise zone (< ±2pp, no real edge either way)
Spread + Moneyline are graded. Player props are not yet graded, so use stats data for those.
PREFER ⭐ picks unless verified data (injury, situational, info edge) overrides the math.
NEVER pick a ❌ side unless you can specifically explain why the math is wrong on this game.
...
2. **ALWAYS reference news article titles** from the "News Insights" section when available
3. **ALWAYS use the PRE-COMPUTED AI GAME ANALYSIS section**. It contains data-backed edge scores, recommended picks, key factors, and records. USE these insights to strengthen your reasoning. Cite specific analysis snippets, edge scores, and key factors.
4. **NEVER make up:** ATS records, injury details, player stats, recent scores, or any data not provided above
5. **For PLAYER PROPS:** You MUST call get_player_stats() to retrieve concrete stats (avg receptions, yards, TDs). If get_player_stats() reports that NO stats are available for a player, you MUST NOT recommend ANY prop for that player. You MUST choose a different player or market with real stats.
...
6. **Example of GOOD reasoning:** "Ravens (9-3) host Bengals (5-7). Ravens -6.5 offers value given their superior record. Pre-computed analysis gives this an edge score of 8.5/10. Recent news: 'Ravens defense dominates in Week 12' suggests strong form. Bengals allow 27ppg per their record."
7. **Example of BAD reasoning:** "Ravens are 8-2 ATS this season" (made-up stat), "Lamar Jackson threw for 300 yards last week" (not provided), "Detailed recent stat splits are not available in the cache" (hedging - skip the pick instead)
...
module.exports = { ParlayAnalyst, ANALYST_MODEL };
