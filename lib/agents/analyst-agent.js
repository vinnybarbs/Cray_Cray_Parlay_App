...
`.trim();
  ...
  async selectBestPicks({ picks, numSuggestions, riskLevel, betTypes, apiKey, realData = null, historicalLessons = [] }) {
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o', // Options: 'gpt-4o' | 'o1-mini' | 'gpt-5-mini' (if released)
        messages: [
          { role: 'system', content: `You are a sports betting analyst. The VERIFIED DATA is provided as compact JSON.

REQUIREMENTS:
- Parse the JSON data to extract team stats, player info, injuries, records
- Use short keys: ppg=points/game, yds=yards, n=name, td=touchdowns, etc.
- Cite specific stats: "Chiefs 28 PPG" not "good offense"
- Reference actual player names from players.qb/rb/wr
- Mention injuries from injuries section by player name
- NO generic reasoning - use the numbers from JSON
- NEVER mention a player not in the JSON data
- NEVER guess or estimate player positions - only use data provided

CRITICAL - For each pick:
- Spread bets: MUST include "point" field with spread value (e.g., +6.5 or -3.5)
- Total bets: MUST include "point" field with line (e.g., 47.5)
- Moneyline bets: Set "point" to null
- "odds" field is ALWAYS the American odds (e.g., -112, +140)

Example good: "Mahomes (qb) 285 yds/game, Chiefs 28 PPG vs Raiders allowing 24"
Example bad: "Good matchup in favorable environment"
  ...
  buildPickSelectionPrompt(picks, numSuggestions, riskLevel, betTypes, realData = null, historicalLessons = []) {
   ...
   ALL PICKS: ${group.picks.map(p => `${p.id}:${p.betType}-${p.pick}@${p.odds}`).join(', ')}`;
    ...
    if (realData) {
      ...
      // Build compact JSON data object
      const compactData = {};
      
      // Injuries (compact)
      if (realData.injuries && Object.keys(realData.injuries).length > 0) {
        compactData.injuries = {};
        Object.entries(realData.injuries).forEach(([team, injuries]) => {
          if (injuries && injuries.length > 0) {
            compactData.injuries[team] = injuries.map(inj => ({
              p: inj.player,
              pos: inj.position,
              s: inj.status,
              i: inj.injury
      ...
      // Team records (compact)
      if (realData.teamRecords && Object.keys(realData.teamRecords).length > 0) {
        compactData.records = {};
        Object.entries(realData.teamRecords).forEach(([team, data]) => {
          if (typeof data === 'object' && data.record) {
            compactData.records[team] = {
              rec: data.record,
              wp: Math.round(data.winPercentage * 100),
              pd: data.pointDifferential,
              str: data.streak,
              rnk: data.divisionRank
            };
          }
        });
      }
      ...
      // Team stats (compact)
      if (realData.teamStats && Object.keys(realData.teamStats).length > 0) {
        compactData.stats = {};
        Object.entries(realData.teamStats).forEach(([team, stats]) => {
          compactData.stats[team] = {
            ppg: stats.averages.points,
            yds: stats.averages.totalYards,
            pass: stats.averages.passingYards,
            rush: stats.averages.rushingYards,
            to: stats.averages.turnovers
          };
        });
      }
      
      // Top performers (compact)
      if (realData.topPerformers && Object.keys(realData.topPerformers).length > 0) {
        compactData.players = {};
        Object.entries(realData.topPerformers).forEach(([team, p]) => {
          compactData.players[team] = {};
          if (p.topPasser) compactData.players[team].qb = { n: p.topPasser.name, y: p.topPasser.yards, td: p.topPasser.touchdowns };
          if (p.topRusher) compactData.players[team].rb = { n: p.topRusher.name, y: p.topRusher.yards, td: p.topRusher.touchdowns };
          if (p.topReceiver) compactData.players[team].wr = { n: p.topReceiver.name, rec: p.topReceiver.receptions, y: p.topReceiver.yards, td: p.topReceiver.touchdowns };
        });
      }
      
      // News insights (compact)
      if (realData.newsInsights && Object.keys(realData.newsInsights).length > 0) {
        compactData.news = {};
        Object.entries(realData.newsInsights).forEach(([team, insight]) => {
          compactData.news[team] = {
            insights: insight.insights,
            sent: insight.sentiment,
            inj: insight.injuries
          };
        });
      }
      
      realDataSection = `
**=== VERIFIED DATA (JSON FORMAT) ===**

${JSON.stringify(compactData, null, 0)}

**Key:**
- injuries: {p:player, pos:position, s:status, i:injury}
- records: {rec:record, wp:win%, pd:pointDiff, str:streak, rnk:rank}
- stats: {ppg:points/game, yds:total, pass:passing, rush:rushing, to:turnovers} (last 3 games avg)
- players: {qb/rb/wr: {n:name, y:yards, td:touchdowns, rec:receptions}}
- news: {insights:bullets, sent:sentiment, inj:injured players}

**STRICT RULES**:
- Use ONLY data from the JSON above
- Cite specific stats: "Chiefs averaging 28 PPG" not "good offense"
- Reference players by name from JSON
