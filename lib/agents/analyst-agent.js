    ...
    this.functionSchemas = [
      {
        ...
        description: 'Get recent performance stats for a specific player. Use this to research player props.',
        parameters: {
          type: 'object',
          properties: {
            playerName: {
              type: 'string',
              description: 'Full player name (e.g., "Matthew Stafford")'
            },
            team: {
              type: 'string',
              description: 'Team name (e.g., "Los Angeles Rams")'
            },
            statType: {
              type: 'string',
      ...
      {
        name: 'get_team_stats',
        description: 'Get recent team performance stats. Use this for spread/moneyline/total analysis.',
        parameters: {
          type: 'object',
          properties: {
            teamName: {
              type: 'string',
              description: 'Team name (e.g., "Carolina Panthers")'
            },
            lastNGames: {
              type: 'number',
              description: 'Number of recent games (default 3)',
              default: 3
            }
          },
          required: ['teamName']
      ...
      },
      {
        ...
        description: 'Get team win-loss record and standings',
        parameters: {
          type: 'object',
          properties: {
            teamName: {
      ...
      {
        name: 'get_injuries',
        description: 'Get current injury report for a team',
        parameters: {
          type: 'object',
          properties: {
            teamName: {
      ...
      },
      {
        ...
        description: 'Get recent betting-relevant news insights for a team',
        parameters: {
          type: 'object',
          properties: {
            teamName: {
...
`.trim();
  ...
  async selectBestPicks({ picks, numSuggestions, riskLevel, betTypes, apiKey, realData = null, historicalLessons = [], supabase = null }) {
    ...
    // NOTE: Disabled for o4-mini - Responses API doesn't support function calling yet
    const aiFunctions = null; // supabase ? new AIFunctions(supabase) : null;
    if (aiFunctions) {
      console.log(`   🔧 Function calling enabled - AI can query database dynamically`);
    } else {
      console.log(`   📊 Using verbose data format (function calling not supported by o4-mini yet)`);
    }
    
    const prompt = this.buildPickSelectionPrompt(picks, numSuggestions, riskLevel, betTypes, realData, historicalLessons);
    
    // System prompt for o4-mini with function calling
...
${aiFunctions ? `
FUNCTION CALLING - YOU CAN QUERY THE DATABASE:
- Use get_player_stats() to research player props (pass TDs, rush yards, receptions, etc.)
- Use get_team_stats() to analyze team performance for spreads/totals
- Use get_team_record() to get win-loss records
- Use get_injuries() to check injury reports
- Use get_news_insights() to get recent betting-relevant news

RESEARCH EACH PICK BEFORE SUGGESTING IT:
1. For player props: Call get_player_stats(playerName, team, statType, lastNGames=5)
2. For team bets: Call get_team_stats(teamName, lastNGames=3)
3. Check injuries: Call get_injuries(teamName)
4. Example: For "Stafford Over 2.5 Pass TDs", call get_player_stats("Matthew Stafford", "Los Angeles Rams", "passing", 5)

DO NOT make generic statements. Research specific data for each pick.
` : `
DATA GUIDELINES:
...
Return valid JSON only.`;
    ...
    while (iteration < maxIterations) {
      ...
      // NOTE: o4-mini Responses API may not support tools yet - disable for now
      if (aiFunctions && false) { // Temporarily disabled
      ...
      const toolCalls = messageOutput?.tool_calls;
      
      // Since function calling is disabled for o4-mini, just get the response
      if (!aiFunctions || !toolCalls || toolCalls.length === 0) {
        // No more function calls - this is the final answer
        finalContent = messageOutput?.content?.[0]?.text;
...
${Object.entries(realData.teamRecords || {}).map(([team, data]) => {
  if (typeof data === 'object' && data.record) {
    return `${team}: ${data.record} (${(data.winPercentage * 100).toFixed(1)}% win rate) | Point Diff: ${data.pointDifferential > 0 ? '+' : ''}${data.pointDifferential} | Streak: ${data.streak || 'N/A'} | Division Rank: #${data.divisionRank || 'N/A'}`;
  }
  return `${team}: ${data.record || `${data.wins}-${data.losses}`}`;
}).join('\n') || 'No records available'}

**RECENT TEAM PERFORMANCE** (Last 3 Games):
${Object.entries(realData.teamStats || {}).map(([team, stats]) => {
  return `${team}: Averaging ${stats.averages.points} PPG, ${stats.averages.totalYards} total yards (${stats.averages.passingYards} pass, ${stats.averages.rushingYards} rush), ${stats.averages.turnovers} turnovers/game`;
}).join('\n') || 'No recent stats available'}

**TOP PERFORMERS** (Season Leaders):
${Object.entries(realData.topPerformers || {}).map(([team, performers]) => {
  const parts = [];
  if (performers.topPasser) parts.push(`  QB ${performers.topPasser.name}: ${performers.topPasser.yards} yards, ${performers.topPasser.touchdowns} TDs`);
  if (performers.topRusher) parts.push(`  RB ${performers.topRusher.name}: ${performers.topRusher.yards} yards, ${performers.topRusher.touchdowns} TDs`);
  if (performers.topReceiver) parts.push(`  WR ${performers.topReceiver.name}: ${performers.topReceiver.receptions} rec, ${performers.topReceiver.yards} yards, ${performers.topReceiver.touchdowns} TDs`);
  return parts.length > 0 ? `${team}:\n${parts.join('\n')}` : '';
}).filter(Boolean).join('\n\n') || 'No performer data available'}

**NEWS INSIGHTS** (Betting-relevant):
${Object.entries(realData.newsInsights || {}).map(([team, insight]) => {
