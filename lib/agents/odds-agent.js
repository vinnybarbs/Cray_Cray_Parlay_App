  ...
  async fetchFromBook(bookmaker, sports, betTypes, now, rangeEnd, { fastMode = false, capCount = 12 } = {}) {
    ...
    if (this.supabase) {
      try {
        ...
        const cachedOdds = await this.fetchFromCache(sports, bookmaker, requestedMarkets);
        if (cachedOdds && cachedOdds.length > 0) {
          console.log(`✅ Using ${cachedOdds.length} games from cache (including props if requested)`);
          return cachedOdds;
        } else {
    ...
    // If no cache, throw error instead of calling live API
    throw new Error('No cached odds available. Please wait for the hourly refresh or contact support.');
