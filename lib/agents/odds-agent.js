...
class TargetedOddsAgent {
   ...
   */
  async fetchFromCache(sports, bookmaker, markets) {
    if (!this.supabase) {
    ...
    try {
      ...
      // Query cache for recent odds (within last 2 hours)
      ...
      if (!data || data.length === 0) {
        console.log('📦 Cache empty - will fetch live');
        return null;
      }
  ...
  async fetchFromBook(bookmaker, sports, betTypes, now, rangeEnd, { fastMode = false, capCount = 12 } = {}) {
    ...
    // Try to get from cache first (only for core markets, not props)
    const coreMarkets = ['h2h', 'spreads', 'totals'];
    const requestingCoreOnly = betTypes.every(bt => 
      ['Moneyline/Spread', 'Totals (O/U)'].includes(bt)
    );
    
    if (requestingCoreOnly && this.supabase) {
      try {
        const cachedOdds = await this.fetchFromCache(sports, bookmaker, coreMarkets);
        if (cachedOdds && cachedOdds.length > 0) {
          console.log(`✅ Using ${cachedOdds.length} games from cache`);
          return cachedOdds;
        }
      } catch (cacheError) {
        console.log(`⚠️ Cache read failed, falling back to live API: ${cacheError.message}`);
      }
    }
