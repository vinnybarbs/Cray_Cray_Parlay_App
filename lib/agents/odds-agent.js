...
class TargetedOddsAgent {
  ...
  calculateDateRange(dateRange) {
    ...
    if (dateRange === 1) {
      ...
      // Treat the max slider value as a "4+ day" window so users can see upcoming games
      const days = 7;
      rangeEnd = new Date(now.getTime() + days * 24 * 60 * 60 * 1000);
      const rangeEndMT = rangeEnd.toLocaleString('en-US', { timeZone: 'America/Denver', hour12: true });
      console.log(`📅 4+ day mode: until ${rangeEnd.toISOString()} (${rangeEndMT} MT)`);
    } else {
