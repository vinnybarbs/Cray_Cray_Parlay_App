...
function validateParlayRequest(req, res, next) {
  ...
  if (!Array.isArray(selectedBetTypes) || selectedBetTypes.length === 0) {
    ...
    const validBetTypes = ['Moneyline/Spread', 'Player Props', 'TD Props', 'Totals (O/U)', 'Team Props',
      'Moneyline', 'Spread', 'Total', 'Over/Under', 'Player Points', 'Player Rebounds', 'Player Assists'];
  ...
  // Validate numLegs (optional for suggest-picks which uses suggestionCount instead)
  if (typeof numLegs !== 'undefined' && (typeof numLegs !== 'number' || numLegs < 1 || numLegs > 10)) {
  ...
  // Validate aiModel (optional; 'anthropic' is the only engine, 'openai'
  // accepted for old clients and ignored)
  if (typeof aiModel !== 'undefined' && aiModel !== 'anthropic' && aiModel !== 'openai') {
    errors.push('aiModel must be "anthropic" if provided');
  }
...
function sanitizeInput(req, res, next) {
  // Ensure body exists and set sensible defaults so clients can omit optional fields
  req.body = req.body || {};
  if (!req.body.selectedSports) req.body.selectedSports = ['NFL'];
  if (!req.body.selectedBetTypes) req.body.selectedBetTypes = ['Moneyline/Spread'];
  if (typeof req.body.numLegs === 'undefined' || req.body.numLegs === null) req.body.numLegs = 3;
  if (!req.body.oddsPlatform) req.body.oddsPlatform = 'DraftKings';
  if (!req.body.riskLevel) req.body.riskLevel = 'Medium';
  if (typeof req.body.dateRange === 'undefined' || req.body.dateRange === null) req.body.dateRange = 1;
  if (!req.body.aiModel) req.body.aiModel = 'anthropic';
