...
const ALIASES = {
  ...
  // Soccer: the odds feed and ESPN name these clubs differently, and
  // neither the mascot rule nor the club-suffix rule bridges them
  // (2026-09-12: four clubs showed no record on the tile).
  'los angeles fc': 'lafc',
...
function normalize(name) {
  if (!name) return '';
  return name
    // Strip diacritics first ("CF Montréal" -> "CF Montreal", "São Paulo" -> "Sao Paulo").
    // ai_suggestions and game_results often disagree on accents because they
...
// Soccer/MLS/EPL/MLB clubs come with suffixes that vary across data sources:
//   "Atlanta United FC" vs "Atlanta United", "Inter Miami CF" vs "Inter Miami",
//   "Seattle Sounders FC" vs "Seattle Sounders", "Nashville SC" vs "Nashville".
// Drop a single trailing club designator so the mascot/word match works.
const CLUB_SUFFIXES = new Set(['fc', 'cf', 'sc', 'afc']);
function stripClubSuffix(normalized) {
  const words = normalized.split(' ');
  if (words.length > 1 && CLUB_SUFFIXES.has(words[words.length - 1])) {
...
function teamsMatch(name1, name2) {
  ...
  // 1b. Exact match after stripping a single trailing club designator
  //     (handles "Atlanta United FC" ↔ "Atlanta United", etc.)
  const s1 = stripClubSuffix(n1);
  const s2 = stripClubSuffix(n2);
  ...
  // 4. Mascot matches, so now require at least ONE more word overlap
