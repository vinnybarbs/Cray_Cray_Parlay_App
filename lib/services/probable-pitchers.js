...
const SCOREBOARD = 'https://site.web.api.espn.com/apis/site/v2/sports/baseball/mlb/scoreboard';
...
function eraFor(probable) {
  // Structured ERA for the edge math. From statistics when present, else
  // parsed out of a "12-7, 3.01 ERA" style record string.
  const stats = Array.isArray(probable?.statistics) ? probable.statistics : [];
  for (const s of stats) {
    if (String(s?.abbreviation).toUpperCase() === 'ERA') {
      const v = parseFloat(s.displayValue);
      if (Number.isFinite(v)) return v;
    }
  }
  const m = String(probable?.record || '').match(/([0-9]+(?:\.[0-9]+)?)\s*ERA/i);
  if (m) {
 ...
 */
async function findEntry(homeTeam, awayTeam) {
...
async function getProbablePitchersText(homeTeam, awayTeam) {
  try {
    const entry = await findEntry(homeTeam, awayTeam);
...
/**
 * Structured starters for the edge MATH (the text above is for the
 * narration prompt). Returns { home: {name, era}, away: {name, era} }
 * with era null when unknown, or null when neither starter is known.
 */
async function getProbablePitcherStats(homeTeam, awayTeam) {
  try {
    const entry = await findEntry(homeTeam, awayTeam);
    if (!entry) return null;
    return {
      home: entry.home ? { name: entry.home.pitcher, era: entry.home.era ?? null } : null,
      away: entry.away ? { name: entry.away.pitcher, era: entry.away.era ?? null } : null,
    };
  } catch {
    return null;
  }
...
module.exports = { getProbablePitchersText, getProbablePitcherStats, starterEraAdjustment, _resetCache };

/**
 * Pure math: home win probability adjustment from the starters' ERA gap.
 * Positive means the HOME starter is better. Roughly 4pp per full run of
 * ERA differential, capped at 6pp, which matches how books price an
 * ace-versus-spot-starter gap. Returns 0 unless BOTH ERAs are known and
 * sane, a one-sided announcement is no evidence about the other side.
 */
function starterEraAdjustment(homeEra, awayEra) {
  const h = Number(homeEra), a = Number(awayEra);
  if (!Number.isFinite(h) || !Number.isFinite(a)) return 0;
  if (h <= 0 || a <= 0 || h > 20 || a > 20) return 0;
  const adj = (a - h) * 0.04;
