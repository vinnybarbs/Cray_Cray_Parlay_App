...
const { sportDayISO, daysAgo } = require('./sport-day');
...
class AISuggestionOutcomeChecker {
  ...
  async checkAllPendingSuggestions({ daysBack = 7 } = {}) {
    try {
      ...
      // Lookback anchored on the Eastern game-day so a late-evening run
      // doesn't shift the window a day forward.
      const cutoff = sportDayISO(daysAgo(daysBack));
