...
class AISuggestionOutcomeChecker {
  ...
  async getGameResult(suggestion) {
    try {
      ...
      // Odds API /scores first — same vocabulary as ai_suggestions, so exact
      // name match works (covers MLS/EPL accents, UFC fight cards, Tennis).
  ...
  async getGameResultFromESPN(suggestion) {
    try {
      ...
      // Also try day before (timezone edge case: MT night game = next day UTC)
      const dayBeforeStr = new Date(gameDate.getTime() - 86400000).toISOString().split('T')[0].replace(/-/g, '');

      // Try both dates (timezone edge case)
      let data;
      for (const d of [dateStr, dayBeforeStr]) {
        const url = `https://site.api.espn.com/apis/site/v2/sports/${sportPath}/scoreboard?dates=${d}&groups=50&limit=200`;
      ...
      const game = data.events?.find(event => {
        ...
        const correctOrder = this.teamsMatch(homeTeam?.team?.displayName, suggestion.home_team) &&
          this.teamsMatch(awayTeam?.team?.displayName, suggestion.away_team);
        const reversedOrder = this.teamsMatch(homeTeam?.team?.displayName, suggestion.away_team) &&
          this.teamsMatch(awayTeam?.team?.displayName, suggestion.home_team);
