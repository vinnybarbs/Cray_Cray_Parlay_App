...
class AISuggestionOutcomeChecker {
  ...
  async checkSuggestionOutcome(suggestion) {
    try {
      ...
      if (!gameResult) {
        // UFC cards scratch and replace fights late, and tennis draws
        // retire, walk over, and slip event ids. A pick in either sport
        // with no score match three days after start time is a bout or
        // match that will never grade. Void it (stake returned, excluded
        // from the record) instead of leaving it pending forever. Tennis
        // joined the rule 2026-09-09 after six orphans sat nine ops checks.
        const isVoidable = ['UFC', 'MMA', 'Tennis'].includes(suggestion.sport);
        const startMs = suggestion.game_date ? new Date(suggestion.game_date).getTime() : null;
        if (isVoidable && startMs && Date.now() - startMs > 3 * 24 * 60 * 60 * 1000) {
          const { error: voidErr } = await this.supabase
            .from('ai_suggestions')
            .update({
              actual_outcome: 'void',
              resolved_at: new Date().toISOString(),
              voided_reason: `Unmatched ${suggestion.sport} result 3+ days after start (scratch, retirement, walkover, or event id gap)`,
            })
            .eq('id', suggestion.id);
          if (!voidErr) {
            logger.info(`✅ AI suggestion ${suggestion.id}: void (unmatched ${suggestion.sport} 3+ days old)`);
  ...
  checkSpreadOutcome(suggestion, scoreDiff) {
    ...
    // `point` is the PICKED SIDE'S OWN line ("Boston Red Sox -1.5" stores
    // -1.5), so covering means own margin PLUS line clears zero. This used
    // to SUBTRACT the line, which handed favorites an extra spread of slack
    // and charged dogs theirs twice: on 2026-08-31 it graded Red Sox -1.5
    // as won on a 1-run win and Rays -1.5 as won on an outright loss, and
    // on 2026-08-26 it graded a covering Braves +1.5 as lost. The SQL
    // grader (determine_outcome) always had it right, so wrong grades
    // landed only when this checker reached a spread first.
  ...
  async getGameResult(suggestion) {
    try {
      ...
      const oddsResult = await this.oddsApiScores.findGameResult({
        ...
        odds_event_id: suggestion.odds_event_id || null,
      });
  ...
  async getGameResultFromDB(suggestion) {
    try {
      // Exact local game date only. The old three-day window let a
      // mid-series pick grade off the previous night's final between the
      // same teams (2026-08-08 Diamondbacks incident). game_results dates
      // are US-local, so localize the pick's timestamp the same way.
      const gameDateStr = new Date(suggestion.game_date)
