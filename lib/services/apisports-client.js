...
class ApiSportsClient {
  constructor(apiKey) {
    ...
    this.callCount = 0;
    this.dailyLimit = 7500; // API-Sports paid tier limit
  }
  ...
  async request(endpoint, params = {}) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`API-Sports error: ${response.status} ${response.statusText} - ${errorBody}`);
      }
  ...
  /**
   * Get ALL games for a season (MOST efficient - one call!)
   */
  async getAllSeasonGames(season = null, league = 1) {
    season = season || this.getCurrentSeason();
    return this.request('games', { league, season });
  }
  ...
  async getGamePlayerStats(gameId) {
    // API-Sports American Football stats endpoints expect `game`, not `id`
    return this.request('games/statistics/players', { game: gameId });
  }
  ...
  async getGameTeamStats(gameId) {
    return this.request('games/statistics/teams', { game: gameId });
  }
