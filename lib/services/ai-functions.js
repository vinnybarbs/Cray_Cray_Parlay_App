...
class AIFunctions {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        return {
          ...
          message: `NO_PLAYER_STATS_AVAILABLE: Do NOT recommend any player prop for ${player.name}. You must choose a different player or market where concrete recent stats are available.`
        };
      }
  ...
  async getTeamStats(teamName, lastNGames = 3) {
    try {
      ...
      // 1) Try to get the true current record from the standings table (canonical source)
