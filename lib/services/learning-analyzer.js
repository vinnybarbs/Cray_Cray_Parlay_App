...
class LearningAnalyzer {
  ...
  async analyzePickOutcome(pick) {
    try {
      ...
      // Post-mortems are batch work with no latency requirement, so by
      // default they queue for the local-model worker on the owner's
      // MacBook (llm_jobs, scripts/local-llm-worker.mjs) instead of
      // burning Opus tokens (owner decision 2026-08-19, cost). Set
      // LOCAL_LLM_QUEUE=false to restore the direct API path.
      if (process.env.LOCAL_LLM_QUEUE !== 'false') {
