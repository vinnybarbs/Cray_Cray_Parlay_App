...
class LearningAnalyzer {
  ...
  async analyzePickOutcome(pick) {
    try {
      ...
      // Use Perplexity API which has built-in web search
      const response = await fetch('https://api.perplexity.ai/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are an expert sports betting analyst who learns from outcomes to improve future predictions. Search the web for actual game recaps and post-game analysis. Analyze picks objectively and extract actionable lessons based on real game data.'
            },
            {
              role: 'user',
              content: prompt
          ...
          temperature: 0.2,
          max_tokens: 1500,
          search_domain_filter: ['espn.com', 'cbssports.com', 'sports.yahoo.com', 'nfl.com', 'nba.com'],
      ...
      if (citations.length > 0) {
        logger.info(`  Found ${citations.length} citations from web search`);
      }
