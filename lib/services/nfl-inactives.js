 ...
 * fetch). Keep only each team's newest snapshot, deduped on the slot
 * key so a single upsert never touches a row twice.
 */
function latestDepthChartRows(rows) {
  ...
  for (const d of rows) {
    ...
    out.set(key, {
      ...
      snapshot_at: new Date(dt).toISOString(),
    });
  }
  return [...out.values()].filter(r => r.player_name);
}
...
module.exports = {
  ...
  mapInjuryRow, latestDepthChartRows, parseSummaryInjuries, diffScratches,
