...
const { sportDayISO, daysAgo } = require('./sport-day.js');
...
class ATSTracker {
  ...
  async recordATS(gameResult) {
    try {
      ...
      if (spread == null) {
        // No spread data, so we can't calculate ATS
        return;
      }
  ...
  async backfillATS(days = 30) {
    try {
      // Eastern game-day cutoff to match game_results.date, which is bucketed
      // the same way.
      const cutoff = sportDayISO(daysAgo(days));
