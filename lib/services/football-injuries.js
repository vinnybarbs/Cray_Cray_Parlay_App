...
async function getFootballInjuryImpact(teamName) {
  ...
  if (!lines) return { impact: 0, out: 0, doubtful: 0, questionable: 0, keyLoss: null, lines: [] };
  ...
  // watch can diff the game day list against it.
  const report = lines
    .filter(l => (STATUS_WEIGHT[String(l.status).toLowerCase()] ?? 0) > 0)
    .map(l => ({ player: l.player, position: l.position, status: String(l.status).toLowerCase() }));
...
async function overrideTeams(overrides) {
  ...
  for (const [teamName, lines] of Object.entries(overrides || {})) {
    ...
    byTeam.set(String(teamName).toLowerCase(), lines.map(l => ({
      player: l.player, position: l.position || '?', status: l.status,
    })));
  }
...
module.exports = { getFootballInjuryImpact, positionImpact, teamImpact, overrideTeams, _resetCache, _setTeams };
