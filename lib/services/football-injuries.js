...
// The depth chart gate (2026-09-15). Week 1 charged Garrett Nussmeier
// (KC, third quarterback) and Sam Ehlinger (DEN, third) the full 6pp,
// and Arizona's four outs led by a second tight end 4.2pp against the
// Chargers' 2.2pp, which moved the Cardinals at Chargers read 2pp
// toward the loser. The gate weights a report line by the player's
// BEST rank on his club's depth chart over the last 28 days of nflverse
...
// Rank 1 costs the full position cost, rank 2 the injury_depth2_weight
// dial (NFL 0.5), rank 3 and below nothing, a player the chart does not
// list injury_depth_unknown_weight (NFL 0.5, a name mismatch must not
// erase a starter).
...
/** Pure math: the gate multiplier for a depth rank. */
function depthWeight(rank, { depth2 = 0.5, unknown = 0.5 } = {}) {
  if (rank == null || !Number.isFinite(Number(rank))) return unknown;
  const r = Number(rank);
  if (r <= 1) return 1;
  if (r === 2) return depth2;
  return 0;
}
...
async function getFootballInjuryImpact(teamName, opts = {}) {
  ...
  const gate = { depth2: opts.depth2Weight ?? 0.5, unknown: opts.unknownWeight ?? 0.5 };
  const lines = raw.map(l => {
    const rank = lookupRank(opts.depthRanks, abbr, l);
    return { ...l, depth_rank: rank, depth_weight: depthWeight(rank, gate) };
  });
...
module.exports = {
  getFootballInjuryImpact, positionImpact, teamImpact, depthWeight, loadDepthRanks, lookupRank, nameKey,
