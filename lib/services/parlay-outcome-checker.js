...
class ParlayOutcomeChecker {
  ...
  async checkAllPendingParlays() {
    try {
      ...
      // Get all pending parlays with their picks from ai_suggestions
      const { data: pendingParlays, error } = await this.supabase
        .from('parlays')
        .select(`
          *,
          ai_suggestions!parlay_id (*)
        `)
        .eq('status', 'pending')
  ...
  async checkParlayOutcome(parlay) {
    try {
      const picks = parlay.ai_suggestions || [];
      logger.info(`Checking parlay ${parlay.id} with ${picks.length} picks`);
