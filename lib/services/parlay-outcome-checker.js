...
class ParlayOutcomeChecker {
  ...
  async getGameResult(leg) {
    try {
      ...
      // Try game_results DB first (populated by backfill cron)
      const dbResult = await this.getGameResultFromDB(leg);
      if (dbResult) return dbResult;

      if (process.env.APISPORTS_API_KEY) {
        const apiResult = await this.getGameResultFromAPISports(leg);
  ...
  async getGameResultFromESPN(leg) {
    try {
      ...
      const espnEndpoints = {
        ...
        'NCAA': 'https://site.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard', // Alias
        'NBA': 'https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard',
        'NCAAB': 'https://site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball/scoreboard',
        'MLB': 'https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/scoreboard',
        'NHL': 'https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/scoreboard',
        'Soccer': 'https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/scoreboard', // English Premier League
      ...
      // groups=50 gets ALL D1 games (not just top conferences), limit=200 for March Madness days
      const response = await fetch(`${endpoint}?dates=${dateStr}&groups=50&limit=200`);
      if (!response.ok) return null;
  ...
  teamsMatch(apiTeamName, legTeamName) {
    ...
    // Normalize: lowercase, expand common abbreviations
    const normalize = (name) => name.toLowerCase()
      .replace(/\bst\b/g, 'state')  // "Michigan St" → "Michigan State"
      .replace(/\bft\b/g, 'fort')
      .replace(/\bmt\b/g, 'mount')
      .replace(/['']/g, "'")
      .trim();

    const n1 = normalize(apiTeamName);
    const n2 = normalize(legTeamName);
