...
class ParlayOutcomeChecker {
  ...
  async getPlayerStatsFromAPISports(gameId, playerName) {
    try {
      ...
      // Search all players for name match
      const allPlayers = result.response;
      const player = allPlayers.find(p => 
        this.playerNamesMatch(p.player.name, playerName)
      );
      ...
      // Parse stats from API-Sports format
      const stats = player.statistics || {};
      
      return {
        // Passing
        passyds: stats.passing?.yards || 0,
        passtd: stats.passing?.touchdowns || 0,
        passint: stats.passing?.interceptions || 0,
        passcomp: stats.passing?.completions || 0,
        passatt: stats.passing?.attempts || 0,
        
        // Rushing
        rushyds: stats.rushing?.yards || 0,
        rushtd: stats.rushing?.touchdowns || 0,
        rushatt: stats.rushing?.attempts || 0,
        
        // Receiving
        rec: stats.receiving?.receptions || 0,
        recyds: stats.receiving?.yards || 0,
        rectd: stats.receiving?.touchdowns || 0,
