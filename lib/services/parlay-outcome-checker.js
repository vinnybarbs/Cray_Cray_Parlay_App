...
class ParlayOutcomeChecker {
  ...
  async checkParlayOutcome(parlay) {
    try {
      logger.info(`Checking parlay ${parlay.id} with ${parlay.parlay_legs.length} legs`);
    ...
    try {
      ...
      switch (leg.bet_type?.toLowerCase()) {
        ...
        case 'moneyline/spread':
          return this.checkMoneylineOutcome(betDetails, scoreDiff, homeScore, awayScore);
          
        case 'spread':
          return this.checkSpreadOutcome(betDetails, scoreDiff, homeScore, awayScore);
          
        case 'total':
        case 'over/under':
          return this.checkTotalOutcome(betDetails, homeScore + awayScore);
   ...
   */
  checkMoneylineOutcome(betDetails, scoreDiff, homeScore, awayScore) {
    const description = betDetails.description?.toLowerCase() || '';
    
    // Determine which team was picked
    let teamWon = false;
    
    if (description.includes('home') || description.includes('ml')) {
      teamWon = scoreDiff > 0; // Home team won
    } else if (description.includes('away')) {
      teamWon = scoreDiff < 0; // Away team won
    } else {
      // Parse team name from description
      // This would need more sophisticated parsing based on your bet format
      return null;
    }
   ...
   */
  checkSpreadOutcome(betDetails, scoreDiff, homeScore, awayScore) {
   ...
   */
  checkTotalOutcome(betDetails, totalScore) {
    const line = betDetails.line || 0;
    const description = betDetails.description?.toLowerCase() || '';
    ...
    try {
      const { error } = await this.supabase
        ...
        .update({
          ...
          outcome: update.result === 'won' ? 'win' : update.result === 'lost' ? 'loss' : 'push',
