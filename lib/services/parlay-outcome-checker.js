...
// Soccer h2h is a THREE-WAY market settled on regulation: a draw (or a
// knockout decided after regulation) means an ML pick on either team LOST.
...
class ParlayOutcomeChecker {
    ...
    try {
      ...
      if ((!picks || picks.length === 0) && parlay.metadata?.locked_picks?.length > 0) {
        ...
        const legsToInsert = parlay.metadata.locked_picks.map((lp, idx) => ({
          ...
          game_date: lp.gameDate ? siteDay(lp.gameDate) : siteDay(),
          sport: lp.sport || 'NFL',
  ...
  async getGameResultFromDB(leg) {
    try {
      // Exact date only. The old three-day window let a mid-series leg
      // grade off the previous night's final between the same teams
      // (2026-08-08 Diamondbacks incident, same bug in three graders).
  ...
  async getPlayerStatsFromESPN(leg, playerName) {
    try {
      ...
      // leg.game_date is a date-only column; round-tripping it through Date()
      // re-derives the day in UTC and shifts evening games. Use the stored
      // site day verbatim (or its site-day conversion if a timestamp sneaks in).
      const dateStr = String(leg.game_date).includes('T')
        ? siteDay(leg.game_date).replace(/-/g, '')
        : String(leg.game_date).replace(/-/g, '');

      const url = `https://site.web.api.espn.com/apis/site/v2/sports/${sportPath}/scoreboard?dates=${dateStr}`;
      ...
      const boxScoreUrl = `https://site.web.api.espn.com/apis/site/v2/sports/${sportPath}/summary?event=${gameId}`;
    ...
    try {
      ...
      switch (betTypeLower) {
        ...
        case 'moneyline/spread':
          return this.checkMoneylineOutcome(leg, betDetails, scoreDiff, homeScore, awayScore, gameResult);
   ...
   */
  checkMoneylineOutcome(leg, betDetails, scoreDiff, homeScore, awayScore, gameResult = null) {
    const pick = leg.pick || '';
    ...
    const isSoccer = SOCCER_SPORTS.has(leg.sport);
    if (isSoccer && gameResult?.wentToExtraTime) {
      // Decided after regulation, so the 3-way market settled as a draw.
      return { result: 'lost', actualValue: 0, marginOfVictory: 0 };
    }
    if (scoreDiff === 0) {
      return { result: isSoccer ? 'lost' : 'push', actualValue: 0, marginOfVictory: 0 };
    }
  ...
  checkSpreadOutcome(leg, betDetails, scoreDiff, homeScore, awayScore) {
    ...
    // The stored point is the PICKED SIDE'S OWN line, so covering means
    // own margin PLUS line clears zero. Subtracting the line (the old
    // code) graded favorites too kindly and dogs too harshly; see the
    // 2026-09-01 spread settlement incident in ai-suggestion-outcome-
    // checker.js for the graded examples.
    const ownMargin = pickedHome ? scoreDiff : -scoreDiff;
    const adjustedDiff = ownMargin + line;
  ...
  /**
   * Check player prop outcome from ESPN box score stats
   */
