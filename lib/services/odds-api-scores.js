 ...
 * or game_results requires fuzzy matching across vocabularies, and that's where
...
// Tennis is dynamic (tournament keys rotate), so it is resolved against odds_cache.
...
class OddsApiScores {
  ...
  async findInTable({ sportKey, sH, sA, targetTime }) {
    // 3 hours, not 36: during a series the same team pair plays on
    // consecutive nights, and a wide window graded tonight's Diamondbacks
    // pick off yesterday's final (2026-08-08). 3h covers schedule jitter
    // while excluding both yesterday and a doubleheader sibling.
    const windowStart = targetTime ? new Date(targetTime - 3 * 60 * 60 * 1000).toISOString() : null;
    const windowEnd = targetTime ? new Date(targetTime + 3 * 60 * 60 * 1000).toISOString() : null;
    let q = this.supabase
      .from('odds_api_scores')
   ...
   */
  async findGameResult({ sport, home_team, away_team, game_date, odds_event_id }) {
    if (!sport || !home_team || !away_team) return null;

    // Step 0: exact event identity. Picks created since 2026-08-09 carry
    // the Odds API event id they were priced from; when it's present, the
    // result lookup is a primary-key hit and no team/date/window matching
    // can ever pull a neighboring game.
    if (odds_event_id) {
      const { data: byId } = await this.supabase
        .from('odds_api_scores')
        .select('home_score, away_score, completed')
        .eq('event_id', odds_event_id)
        .eq('completed', true)
        .maybeSingle();
      if (byId && byId.home_score != null && byId.away_score != null) {
        return {
          homeScore: byId.home_score,
          awayScore: byId.away_score,
          status: 'completed',
          source: 'odds_api_event_id',
        };
      }
    }
    ...
    // Step 1: check the persisted cache table. It covers games beyond /scores'
    ...
    for (const key of keys) {
      ...
      for (const game of scores) {
        ...
        // Same-game guard: /scores returns several days of finals and a
        // team pair mid-series appears more than once. Only a start time
        // within 3 hours of the pick's is the same game (2026-08-08
        // Diamondbacks incident). No timestamp means no match.
        if (!targetTime || !game.commence_time) continue;
