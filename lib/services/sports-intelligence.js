...
class SportsIntelligenceService {
  ...
  async getTeamIntelligence(sport, teamName) {
    try {
      ...
      const rssArticles = await this.getTeamRssArticles(teamName);
      const intelligence = this.organizeTeamIntelligence(data || [], rssArticles);
  ...
  async getTeamRssArticles(teamName) {
    try {
      if (!teamName) return [];
      ...
      const { data, error } = await supabase
        .from('news_articles')
        .select('id, title, link, summary, published_at, betting_summary, injury_mentions, sentiment')
      ...
      if (error || !data) {
        if (error) {
          logger.error('Error fetching RSS news for team:', { teamName, error });
        }
        return [];
      }
      ...
      return data.map(article => ({
        ...
        bettingSummary: article.betting_summary || null,
        injuries: article.injury_mentions || null,
      ...
      logger.error('SportsIntelligenceService.getTeamRssArticles error:', error);
      return [];
    }
  }
   ...
   */
  organizeTeamIntelligence(rawData, rssArticles = []) {
    const organized = {
      injuries: null,
      analystPicks: null,
      teamNews: null,
    ...
    if (rssArticles && rssArticles.length > 0) {
      const rssIntel = {
        summary: 'Latest headlines and storylines from curated news feeds',
        articles: rssArticles,
        lastUpdated: rssArticles[0].date || new Date().toISOString(),
        tagline: '📰 Latest headlines from trusted news feeds'
      };

      if (organized.teamNews) {
        const existing = organized.teamNews;
        const combinedArticles = [...rssArticles, ...(existing.articles || [])];
        organized.teamNews = {
          summary: existing.summary || rssIntel.summary,
          articles: combinedArticles,
          lastUpdated: existing.lastUpdated || rssIntel.lastUpdated,
          tagline: existing.tagline || rssIntel.tagline
        };
      } else {
        organized.teamNews = rssIntel;
        organized.hasIntelligence = true;
      }
    }
  ...
  /**
   * Get league leaders from ESPN for additional context (currently NFL-only)
   */
  async getLeagueLeaders(sport) {
    try {
      ...
      const url = 'https://site.web.api.espn.com/apis/site/v3/sports/football/nfl/leaders';
      logger.info('Fetching league leaders from ESPN', { sport: upper, url });

      const response = await fetch(url);
      if (!response.ok) {
        logger.error('ESPN leaders request failed', { status: response.status, statusText: response.statusText });
        return null;
      }

      const data = await response.json();
      const leadersIntel = this.formatLeadersData(data);
      ...
      logger.error('Error fetching league leaders from ESPN', { error: error.message });
      return null;
    }
  }

  /**
   * Convert raw ESPN leaders payload into short summary lines
   */
  formatLeadersData(data) {
    try {
      if (!data || !Array.isArray(data.categories)) {
        return null;
      }

      const interestingCategories = ['passingYards', 'rushingYards', 'receivingYards'];
      const summaryLines = [];

      data.categories.forEach((category) => {
        if (!interestingCategories.includes(category.name)) return;

        const displayName = category.displayName || category.name;
        const ranks = Array.isArray(category.ranks) ? category.ranks : [];

        const top = ranks.slice(0, 3).map((rank) => {
          const athlete = rank.athlete || rank.player || {};
          const name = athlete.displayName || athlete.shortName || athlete.name;
          const statObj = Array.isArray(rank.stats) && rank.stats.length > 0 ? rank.stats[0] : null;
          const statValue = statObj && (statObj.displayValue || statObj.value);
          if (!name || statValue == null) return null;
          return `${name} (${statValue})`;
        }).filter(Boolean);

        if (top.length > 0) {
          summaryLines.push(`${displayName}: ${top.join(', ')}`);
        }
      });
      ...
      logger.error('Error formatting league leaders data', { error: error.message });
      return null;
    }
  }
  ...
  async getAgentContext(sport, homeTeam, awayTeam) {
    try {
      ...
      // Add high-level league leader context (NFL only for now)
      const leadersIntel = await this.getLeagueLeaders(sport);
      if (leadersIntel && Array.isArray(leadersIntel.summaryLines) && leadersIntel.summaryLines.length > 0) {
        leadersIntel.summaryLines.forEach((line) => {
          contextLines.push(`🏆 League Leaders: ${line}`);
        });
      }

      // Add enriched article intelligence (betting summaries from scraped articles)
      const allArticles = [
        ...(intelligence.homeTeam?.news || []),
        ...(intelligence.awayTeam?.news || [])
      ];
      const enrichedArticles = allArticles.filter(a => a.bettingSummary && a.bettingSummary !== 'Not analyzed');
      if (enrichedArticles.length > 0) {
        const articleContext = enrichedArticles.slice(0, 3).map(a => {
          let line = `📰 ${a.title}`;
          if (a.bettingSummary) line += `. ${a.bettingSummary}`;
          if (a.injuries?.length > 0) {
            const injList = a.injuries.map(i => `${i.player} (${i.status})`).join(', ');
            line += ` | Injuries: ${injList}`;
          }
          return line;
        });
        contextLines.push(...articleContext);
      }
