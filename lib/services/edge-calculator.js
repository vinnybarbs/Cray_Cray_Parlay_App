...
// MARKET ANCHOR threshold (owner rule 2026-09-02: "stop thinking odds is
// purely a break even number, it's an indicator of likelihood"). Under
// this many COMBINED games played, a football matchup's base probability
// is the devigged consensus implied probability, not the record blend:
// teams open the season 0-0, the record base falls toward 50/50, and the
// gap to a 93/7 market printed as fake 40pp longshot edges (the +1400
// NCAAF class). Anchored games can only earn edge through the factor
// adjustments, inside a tight cap, so a no-data game reads as a no-edge
// game, which is the honest answer. Retires per sport when the preseason
// strength prior ships (the CFBD build).
...
class EdgeCalculator {
  ...
  async calculateEdge(game) {
    ...
    // Market anchor: see MARKET_ANCHOR_MIN_GAMES. The consensus price
    // across the books IS the likelihood estimate until the season
    // sample is real.
    ...
    const marketAnchored = FOOTBALL_SPORTS.has(sportName)
    ...
    // ------------------------------------------------------------------
    // Anchored games move off the market only through factors, and thin
    // early-season factors do not earn a 15 point leash: ±6pp keeps any
    // anchored read inside Lean-to-Play territory by construction.
    const MAX_NET_ADJUSTMENT = marketAnchored ? 0.06 : 0.15;
