...
class EdgeCalculator {
  ...
  async calculateEdge(game) {
    ...
    if (FOOTBALL_SPORTS.has(sportName)) {
      try {
        ...
        const restAdj = EdgeCalculator.restAdjustment(gameIso, homeLast, awayLast);
        if (restAdj !== 0) {
          homeWinProb += restAdj;
          adjustments.push({
    ...
    if (sigmaTotal > 0 && bookTotal != null && hasScoring) {
      ...
      if (FOOTBALL_SPORTS.has(sportName)) {
        try {
          ...
          const penalty = footballTotalPenalty(row);
          if (row) {
            weatherInfo = {
   ...
   */
  static restAdjustment(gameIso, homeLastIso, awayLastIso) {
    ...
    const adj = Math.max(-0.025, Math.min(0.025, (homeRest - awayRest) * 0.004));
