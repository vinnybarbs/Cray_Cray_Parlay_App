...
class EdgeCalculator {
  ...
  async calculateEdge(game) {
    ...
    const homeFormAdj = this._recentFormAdjustment(homeRecord, homeFormData) * FORM_WEIGHT;
    const awayFormAdj = this._recentFormAdjustment(awayRecord, awayFormData) * FORM_WEIGHT;
    ...
    if (sportName === 'MLB') {
      try {
        ...
        const pitcherAdj = starterEraAdjustment(starters?.home?.era, starters?.away?.era)
          * (marketAnchored ? PITCHER_ANCHOR_DAMP : 1);
        if (pitcherAdj !== 0) {
    ...
    const MAX_NET_ADJUSTMENT = thinFootballAnchor ? 0.06 : 0.15;
    ...
    if (sigma > 0 && homeSpread != null) {
      modelMargin = this._dampedModelMargin(homeWinProb, marketAnchored ? impliedHomeProb : null, sigma);
   ...
   * full transform.
   */
  _dampedModelMargin(winProb, marketProb, sigma) {
    ...
    return market + SPREAD_CLAIM_DAMP * (full - market);
  }
  ...
  _venueSplitImpact(deltaWinPct, totalGames, sport) {
    if (totalGames < 5) return 0;
    const weight = sport === 'MLB' ? 0.3125 : 0.25;
  ...
  _venueSplitAdjustments(homeStandings, awayStandings, sport, homeTeamName, awayTeamName) {
    ...
    if (homeStandings?.home_record && homeStandings?.record) {
      ...
      if (overall && atHome && overall.totalGames >= 10 && atHome.totalGames >= 5) {
        ...
        const impact = this._venueSplitImpact(delta, atHome.totalGames, sport);
    ...
    if (awayStandings?.away_record && awayStandings?.record) {
      ...
      if (overall && onRoad && overall.totalGames >= 10 && onRoad.totalGames >= 5) {
        ...
        const impact = this._venueSplitImpact(delta, onRoad.totalGames, sport);
        if (Math.abs(impact) >= 0.005) {
