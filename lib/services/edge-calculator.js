...
class EdgeCalculator {
  ...
  async calculateEdge(game) {
    ...
    // ------------------------------------------------------------------
    // 1b. Hard bail: if we have NO foundational signal at all (no team
    //     records, no standings) we have no business publishing a model
    //     probability. The historic code defaulted homeWinPct/awayWinPct
    //     to 0.5 in this case — that produced a 50/50 baseline which,
    //     combined with extreme moneylines (tennis: Sinner −10000 vs
    //     Pellegrino +2500), creates fake +45pp "Sharp Takes" on the
    //     longshot and parallel −45pp "Traps" on the favorite.
    //
    //     Returning null tells pre-analyze to store edges: null, which
    //     the digest treats as "no model edge" → the game falls into
    //     "On the bubble" automatically. Honest behavior.
    //
    //     This currently kills edge analysis for: Tennis (player names
    //     not seeded in team_results), UFC (same), Soccer matches whose
    //     teams aren't in standings. That's correct — we don't have a
    //     model for those, and pretending otherwise was the bug.
    // ------------------------------------------------------------------
    if (!homeRecord && !awayRecord && !homeStandings && !awayStandings) {
      return null;
    }
    ...
    if (impliedHomeProb !== null) {
      homeMlEdge = clampEdge(homeWinProb - impliedHomeProb);
      awayMlEdge = clampEdge(awayWinProb - impliedAwayProb);
    ...
    if (sigma > 0 && homeSpread != null) {
      ...
      homeSpreadEdge = clampEdge(homeCoverProb - 0.5);
      awaySpreadEdge = clampEdge(-(homeCoverProb - 0.5));
    }

    // Totals stay null for now (Phase 4C — needs per-team PPG, not just diff).
    const overEdge = null;
    const underEdge = null;
