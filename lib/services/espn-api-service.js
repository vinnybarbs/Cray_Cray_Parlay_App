...
class ESPNApiService {
  ...
  async populateSportRosters(sport, config) {
    ...
    try {
      ...
      // Step 2: Process each team to get roster
      for (const team of teams.slice(0, 5)) { // Start with 5 teams per sport for testing
        try {
  ...
  async insertTeamRoster(team, rosterGroups, sport) {
    try {
      ...
      // First, ensure we have the team in the teams table
      const teamRecord = {
      ...
      console.log(`  💾 Successfully inserted ${adaptedRecords.length} players into database`);
      return adaptedRecords.length;
