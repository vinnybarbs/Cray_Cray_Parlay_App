 ...
 *     -> 'home_spread'   (Lakers IS the home team, line is +10.5, and that's
 *                         the home cover side)
 *
 ...
 *   - signedEdge: number (signed) or null, where positive = model value
...
/**
 * Tier label for a signed pp value. Must stay in lockstep with edgeTier()
 * in src/pages/DailyDigest.jsx and the CASE in the edge-snapshot migration.
 * This is the backend copy used when persisting picks.
 */
// Sharp Take chalk fence (2026-08-10, owner decision): no Sharp Take
// heavier than -150. Break-even at -150 is 60 percent, and 45 days of
// data showed chalk Sharp Takes winning 67 percent for +2.8u while dog
// Sharp Takes won 65 percent for +25.3u, the claimed edges on heavy
// chalk are mostly vig. A fenced pick keeps its research and publishes
// as Play. The fence retires when per-band price calibration ships at
// football go-live and chalk edges haircut themselves honestly.
const SHARP_TAKE_PRICE_FENCE = -150;

// Longshot tier ceiling (owner rule 2026-09-02): a side priced +300 or
// longer is never labeled above Lean, whatever the claimed edge. "I
// don't care what edge exists, no name college teams playing SEC power
// houses at +1300 is not a strong pick period." Every market we have
// measured agrees with the rule: MLB dogs claiming 4pp+ hit 37 percent
// in August, the tennis +251 bucket was pure price-dispersion mirage
// and is fenced from publishing entirely. The chalk fence rails one end
// of the price axis, this rails the other. Must stay in lockstep with
// src/lib/tiers.js.
const LONGSHOT_TIER_CEILING_ODDS = 300;

function edgeTier(signedPp, americanOdds = null) {
  if (signedPp == null || Number.isNaN(Number(signedPp))) return null;
  // Trap is a directional call at 2pp or more below fair, the mirror of
  // the +2pp Lean gate. The -2 to +2 band is noise and grades Skip. Rows
  // stamped before 2026-07-23 used the old below-0 boundary and keep
  // their stored tier, the record is append only.
  if (signedPp <= -2) return 'Trap';
  if (signedPp < 2) return 'Skip';
  const o = americanOdds != null ? Number(String(americanOdds).replace(/[^0-9-]/g, '')) : null;
  if (o != null && Number.isFinite(o) && o >= LONGSHOT_TIER_CEILING_ODDS) return 'Lean';
  if (signedPp < 4) return 'Lean';
  if (signedPp < 7) return 'Play';
  // Strong Play restored 2026-08-16 (owner decision): 7pp is the owner's
  // personal betting floor, so the 7-10 band carries its own label again
  // and per-band calibration owns making the ladder a true cascade.
  // (Merged 2026-08-10 to 2026-08-16, rows stamped 'Play' in that window
  // may carry 7-10pp edges, the record is append only.)
  if (signedPp < 10) return 'Strong Play';
  // Fenced 10pp+ chalk drops one rung to Strong Play, keeping the no-heavy-
  // chalk rule on the flagship tier without hiding a double-digit edge.
  if (o != null && Number.isFinite(o) && o < 0 && o <= SHARP_TAKE_PRICE_FENCE) return 'Strong Play';
  return 'Sharp Take';
}
