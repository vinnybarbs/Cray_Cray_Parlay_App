...
function edgeTier(signedPp, americanOdds = null) {
  ...
  // Strong Play merged into Play 2026-08-10 (owner decision): 4-7 and
  // 7-10 were statistically indistinguishable over every window (30d:
  // 46.5 vs 52.6 on ~100 picks each) and both carried negative CLV, so
  // the split labeled noise as signal. One 4-10 band doubles the sample
  // feeding weekly calibration. Historical Strong Play rows keep their
  // stored tier, the record is append only.
  if (signedPp < 10) return 'Play';
  const o = americanOdds != null ? Number(String(americanOdds).replace(/[^0-9-]/g, '')) : null;
  if (o != null && Number.isFinite(o) && o < 0 && o <= SHARP_TAKE_PRICE_FENCE) return 'Play';
  return 'Sharp Take';
}
