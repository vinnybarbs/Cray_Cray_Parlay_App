 ...
 * in src/lib/tiers.js and the CASE in the edge-snapshot migration. This is
 * the backend copy used when persisting picks.
 *
 * The tier is nothing but the pp band. Price used to move labels here (the
 * -150 chalk fence withheld Sharp Take, the +300 longshot ceiling capped
 * at Lean); since 2026-09-10 both rails are pp deductions on the dial
 * board (lib/services/price-penalties.js, chalk_penalty_pp and
 * longshot_penalty_pp) applied to the claim BEFORE it reaches this
 * ladder, so the published edge_pp already carries them and the record
 * scores exactly what was claimed. Owner: "doesn't matter what label we
 * give it, we score ourselves off pp." Callers that still pass odds as a
 * second argument are ignored on purpose: apply the penalty to the pp,
 * never to the label.
 */
