...
class ApiSportsSync {
   ...
   */
  async syncTeams(league = 1) {
    const leagueName = league === 1 ? 'nfl' : 'ncaaf';
    logger.info(`🏈 Syncing ${leagueName.toUpperCase()} teams...`);

    try {
      const result = await this.apiClient.getTeams(league);
  ...
  async syncInjuries(season = null, league = 1) {
    const leagueName = league === 1 ? 'nfl' : 'ncaaf';
    season = season || this.apiClient.getCurrentSeason();
    logger.info(`🏥 Syncing ${leagueName.toUpperCase()} injuries for ${season}...`);
    ...
    try {
      ...
      const result = await this.apiClient.getInjuries(season, league);
