...
class ApiSportsSync {
  ...
  /**
   * Sync recent game statistics (team + player stats)
   * Optimized: fetches games by week, then stats per game
   */
  async syncRecentGameStats(numWeeks = 2, season = null, league = 1) {
    const leagueName = league === 1 ? 'nfl' : 'ncaaf';
    season = season || this.apiClient.getCurrentSeason();
    logger.info(`📊 Syncing game stats for last ${numWeeks} weeks of ${leagueName.toUpperCase()}...`);
    ...
    try {
      ...
      const { data: latestStat } = await this.supabase
      ...
      const lastSyncedWeek = latestStat?.week || 0;
      const currentWeek = 13; // TODO: Get from standings or API
      
      // If numWeeks is > 10, assume it's a backfill (fetch from week 1)
      // Otherwise, fetch only new weeks since last sync
      let startWeek;
      if (numWeeks > 10) {
        // Backfill mode: start from week 1
        startWeek = 1;
        logger.info(`  Backfill mode: syncing weeks 1-${currentWeek}`);
      } else {
        // Incremental mode: only fetch new weeks
        startWeek = Math.max(lastSyncedWeek + 1, currentWeek - numWeeks + 1);
        logger.info(`  Incremental mode: last synced week ${lastSyncedWeek}, fetching weeks ${startWeek}-${currentWeek}`);
      }
      ...
      // Fetch games week by week (much more efficient!)
      for (let week = startWeek; week <= currentWeek; week++) {
        try {
          logger.info(`  Fetching games for week ${week}...`);
          const gamesResult = await this.apiClient.getGamesByWeek(week, season, league);
          
          if (!gamesResult.response || gamesResult.response.length === 0) {
            logger.info(`    No games found for week ${week}`);
            continue;
          }

          // Process each completed game
          const completedGames = gamesResult.response.filter(g => g.game.status.short === 'FT');
          ...
          for (const game of completedGames) {
            ...
            try {
              // Fetch team stats for this game (gets both teams)
              ...
              // Fetch player stats for this game (gets all players)
              ...
              if (playerStatsResult.response && playerStatsResult.response.length > 0) {
                for (const playerData of playerStatsResult.response) {
                  try {
                    const dbTeam = teamMap.get(playerData.team.id);
                    if (!dbTeam) continue;
                    ...
                    const { data: existingPlayer } = await this.supabase
                      .from('players')
                      .select('id')
                      .eq('apisports_id', playerData.player.id)
                      .eq('league', leagueName)
                      .maybeSingle();
                    ...
                    if (existingPlayer) {
                      ...
                      const { data: newPlayer } = await this.supabase
                        .from('players')
                        .insert({
                          apisports_id: playerData.player.id,
                          name: playerData.player.name,
                          team_id: dbTeam.id,
                          position: playerData.player.position || null,
                    ...
                    await this.supabase
                      ...
                      .upsert({
                        ...
                        opponent_team_id: null, // Could determine from game data
                        passing_attempts: stats.passing?.attempts || null,
                        passing_completions: stats.passing?.completions || null,
                        passing_yards: stats.passing?.yards || null,
                        passing_touchdowns: stats.passing?.touchdowns || null,
                        interceptions: stats.passing?.interceptions || null,
                        rushing_attempts: stats.rushing?.attempts || null,
                        rushing_yards: stats.rushing?.yards || null,
                        rushing_touchdowns: stats.rushing?.touchdowns || null,
                        receptions: stats.receiving?.receptions || null,
                        receiving_yards: stats.receiving?.yards || null,
                        receiving_touchdowns: stats.receiving?.touchdowns || null,
                        targets: stats.receiving?.targets || null,
                        tackles: stats.defense?.tackles || null,
                        sacks: stats.defense?.sacks || null
                      }, {
                        onConflict: 'player_id,game_id'
                      });
                    ...
                    logger.error(`Error storing player ${playerData.player?.name}:`, playerError.message);
                  }
                }
              }
          ...
          logger.error(`Error syncing week ${week}:`, weekError.message);
        }
      }
   ...
   * Fetches last 2 weeks of games - optimized to use minimal API calls
   */
  async weeklySync(season = null, league = 1) {
    ...
    const results = {
      gameStats: await this.syncRecentGameStats(2, season, league), // Last 2 weeks
    };
