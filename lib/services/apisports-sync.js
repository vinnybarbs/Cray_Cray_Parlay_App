...
class ApiSportsSync {
  ...
  /**
   * Sync recent game statistics for all teams
   * Fetches last N games for each team and stores team stats per game
   */
  async syncRecentGameStats(numGames = 3, season = null, league = 1) {
    const leagueName = league === 1 ? 'nfl' : 'ncaaf';
    season = season || this.apiClient.getCurrentSeason();
    logger.info(`📊 Syncing recent game stats (last ${numGames} games) for ${leagueName.toUpperCase()}...`);
    ...
    try {
      ...
      for (const team of teams) {
        try {
          ...
          const gamesResult = await this.apiClient.getTeamGames(team.apisports_id, season);
          
          if (!gamesResult.response || gamesResult.response.length === 0) {
            logger.info(`  No games found for ${team.name}`);
            continue;
          }

          // Get last N completed games (status: Finished)
          const completedGames = gamesResult.response
            .filter(game => game.game.status.short === 'FT')
            .slice(0, numGames);

          for (const game of completedGames) {
            try {
              // Fetch team statistics for this game
              const gameStatsResult = await this.apiClient.getGameTeamStats(game.game.id);
              
              if (!gameStatsResult.response || gameStatsResult.response.length === 0) continue;

              // Find this team's stats in the response
              const teamStats = gameStatsResult.response.find(
                stat => stat.team.id === team.apisports_id
              );
              ...
              logger.error(`Error processing game ${game.game.id} for ${team.name}:`, gameError.message);
            }
          }

          logger.info(`  ✓ Processed ${completedGames.length} games for ${team.name}`);
        } catch (error) {
          logger.error(`Error syncing games for ${team.name}:`, error.message);
        }
      }

      logger.info(`  ✅ Processed ${gamesProcessed} game stats`);
      await this.logSync('game_stats', leagueName, season, 'completed', gamesProcessed);
      return { synced: gamesProcessed };
   ...
   * Fetches last 3 games for each team to get recent performance trends
   */
  async weeklySync(season = null, league = 1) {
    ...
    const results = {
      gameStats: await this.syncRecentGameStats(3, season, league), // Last 3 games
    };
