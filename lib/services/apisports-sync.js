              ...
              for (const group of teamData.groups) {
                ...
                for (const playerEntry of group.players) {
                  try {
                    ...
                    // Load existing stats for this player/game so we can merge groups
                    ...
                    // Initialize from existing values (so later groups don't overwrite unrelated stats)
                    let passing_attempts = existing?.passing_attempts ?? null;
                    let passing_completions = existing?.passing_completions ?? null;
                    let passing_yards = existing?.passing_yards ?? null;
                    let passing_touchdowns = existing?.passing_touchdowns ?? null;
                    let interceptions = existing?.interceptions ?? null;
                    let rushing_attempts = existing?.rushing_attempts ?? null;
                    let rushing_yards = existing?.rushing_yards ?? null;
                    let rushing_touchdowns = existing?.rushing_touchdowns ?? null;
                    let receptions = existing?.receptions ?? null;
                    let receiving_yards = existing?.receiving_yards ?? null;
                    let receiving_touchdowns = existing?.receiving_touchdowns ?? null;
                    let targets = existing?.targets ?? null;
                    let tackles = existing?.tackles ?? null;
                    let sacks = existing?.sacks ?? null;

                    // Update only the fields relevant to this stat group
                    if (groupName.includes('passing')) {
                      const attempts = parseStatValue(stats['attempts']);
                      const completions = parseStatValue(stats['completions']);
                      const yards = parseStatValue(stats['yards']);
                      const tds = parseStatValue(stats['td'] || stats['touchdowns']);
                      const ints = parseStatValue(stats['interceptions'] || stats['int']);

                      if (attempts !== null) passing_attempts = attempts;
                      if (completions !== null) passing_completions = completions;
                      if (yards !== null) passing_yards = yards;
                      if (tds !== null) passing_touchdowns = tds;
                      if (ints !== null) interceptions = ints;
                    } else if (groupName.includes('rushing')) {
                      const carries = parseStatValue(stats['carries'] || stats['rushing attempts']);
                      const rushYards = parseStatValue(stats['rushing yards'] || stats['yards']);
                      const rushTds = parseStatValue(stats['rushing td'] || stats['td']);

                      if (carries !== null) rushing_attempts = carries;
                      if (rushYards !== null) rushing_yards = rushYards;
                      if (rushTds !== null) rushing_touchdowns = rushTds;
                    } else if (groupName.includes('receiving')) {
                      const recs = parseStatValue(stats['receptions'] || stats['rec']);
                      const recYards = parseStatValue(stats['receiving yards'] || stats['yards']);
                      const recTds = parseStatValue(stats['receiving td'] || stats['td']);
                      const tgts = parseStatValue(stats['targets'] || stats['tgt']);

                      if (recs !== null) receptions = recs;
                      if (recYards !== null) receiving_yards = recYards;
                      if (recTds !== null) receiving_touchdowns = recTds;
                      if (tgts !== null) targets = tgts;
                    }

                    // Defense / generic stats (may appear in any group)
                    const tks = parseStatValue(stats['tackles']);
                    const sks = parseStatValue(stats['sacks']);
                    if (tks !== null) tackles = tks;
                    if (sks !== null) sacks = sks;
