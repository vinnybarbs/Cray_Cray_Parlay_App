 ...
 */
/**
 * Is this a UFC-brand bout our settlement pipeline can actually grade?
 * (Owner rule 2026-08-31: non-UFC MMA cards never enter the leg pool.)
 *
 * The MMA odds feed carries every promotion, but ESPN results cover
 * UFC-brand events only, so a leg on a PFL or regional card has no
 * settlement path and dies in the 3-day auto-void as a silent loss of
 * whatever it would have graded (id 16861, Barrett vs Harris). Both
 * fighters having a ufc_fighters row is the practical test for a
 * UFC-brand bout. Deliberately FAIL-CLOSED: a transient query error or
 * a suffix-mangled name skips the leg, because a missing leg is a
 * no-op while an unsettleable one corrupts the pool.
 */
async function isKnownUfcBout(supabase, homeName, awayName) {
  ...
  try {
    ...
    const found = new Set(data.map((r) => r.fighter_key));
    return found.has(homeKey) && found.has(awayKey);
  } catch {
    return false;
  }
}
