  ...
  try {
    ...
    const [rankRes, matchRes] = await Promise.all([
      ...
      supabase
        ...
        // Deterministic tiebreaker. match_date is a DATE and tournament
        // play stacks many same-day matches, so without a secondary key
        // the 60-row cut and the recent-results order shuffled between
        // runs, churned the context hash, and collapsed the tennis change
        // gate (skip rate 40 pct to 9 pct as the draw doubled). Same bug
        // class as the MLB news ordering fixed 2026-08-16.
