...
class ESPNScoreboardService {
  ...
  async fetchYesterdaysGames() {
    // "Yesterday" as a US Eastern game-day, not server-local time.
    const yesterday = daysAgo(1);

    const activeSports = ['NFL', 'NBA', 'NCAAB', 'MLB', 'NHL'];
  ...
  /**
   * Format date as YYYYMMDD for ESPN API (Eastern game-day bucket)
   * @param {Date} date
   * @returns {string}
   */
  formatDate(date) {
    return sportDayCompact(date);
  }
