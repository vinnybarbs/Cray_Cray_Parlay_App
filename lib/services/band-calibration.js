...
//
// The map has a MARKET dimension (2026-08-31, band-aware totals
// re-entry): a (sport, market) fit overrides the sport's pooled
// '__all__' fit for that market's sides. MLB totals carry their own
// shadow-fit map because their delivery is non-monotone in claim size
// (mid bands earn, the 10pp+ tail is an anti-signal), which the pooled
// fit cannot express.
async function loadRawMaps() {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('sport, market, claimed_center, calibrated_center')
      .order('claimed_center', { ascending: true });
    ...
    for (const r of data) {
      ...
      const calibrated = Number(r.calibrated_center);
      if (!Number.isFinite(claimed) || !Number.isFinite(calibrated)) continue;
      const key = `${r.sport || POOLED}|${r.market || POOLED}`;
      if (!bySport.has(key)) bySport.set(key, []);
      bySport.get(key).push({ claimed, calibrated });
    }
...
function sideMarket(side) {
  if (side === 'over' || side === 'under') return 'total';
  if (side === 'home_spread' || side === 'away_spread') return 'spread';
  return 'ml';
}

async function rawPointsFor(sport, market = POOLED) {
  const maps = await loadRawMaps();
  if (!maps) return null;
  return maps.get(`${sport}|${market}`) || maps.get(`${sport}|${POOLED}`) || null;
}
...
async function applyToEdgeData(edgeData, sport = POOLED) {
  ...
  //
  // MUTED MARKETS STAY MUTED: a flat multiplier of 0 is an owner-level
  // market mute (MLB totals, until the shadow crosses its re-entry bar),
  // enforced by the k-scaled edge sitting at 0 and failing the gate. So
  // a side is only re-sized from raw when its flat-k edge is still
  // POSITIVE. Day one of the promotion (2026-08-31) skipped this check
  // and published two totals Sharp Takes from the muted market inside
  // the hour.
  const hasRawFit = (await rawPointsFor(sport)) != null;
  if (hasRawFit && edgeData.edgesRaw && typeof edgeData.edgesRaw === 'object'
      && edgeData.edges && typeof edgeData.edges === 'object') {
    const mapRaw = (points, raw) => {
      const mapped = interpolatePp(points, Number(raw) * 100) / 100;
      return Math.max(-MAX_EDGE, Math.min(MAX_EDGE, mapped));
    };
    for (const [side, raw] of Object.entries(edgeData.edgesRaw)) {
      const current = edgeData.edges[side];
      if (raw != null && Number.isFinite(Number(raw)) && raw > 0
          && current != null && Number.isFinite(Number(current)) && current > 0) {
        const points = await rawPointsFor(sport, sideMarket(side));
        if (points) edgeData.edges[side] = mapRaw(points, raw);
      }
    }
    ...
    if (edgeData.edge != null && Number.isFinite(Number(edgeData.edge)) && edgeData.edge > 0) {
      ...
      if (rawTop != null && Number.isFinite(Number(rawTop)) && rawTop > 0) {
        const mlPoints = await rawPointsFor(sport, 'ml');
        if (mlPoints) edgeData.edge = mapRaw(mlPoints, rawTop);
      }
    }
...
function _setRawPoints(points, sport = POOLED, market = POOLED) {
  ...
  _rawCache.bySport.set(`${sport}|${market}`, points);
}
