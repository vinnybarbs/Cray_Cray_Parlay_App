...
/**
 * Pure: the side keys of every MUTED market for a sport, given its flags.
 * A whole-sport shadow (every market 0) returns nothing: its reads still
 * show on the board as shadow. Since 2026-09-21 a mute is a publish dial
 * at 0 and the multiplier is 1, so a muted market's edge is no longer
 * zero and the headline picker must skip it explicitly, or a muted total
 * becomes the read and blocks the moneyline (the 2026-09-21 leak: eleven
 * MLB headlines landed on Over or Under with totals muted).
 */
function mutedSides(flags) {
  if (!flags) return new Set();
  const off = MARKETS.filter(m => flags[m] === 0);
  if (off.length === 0 || off.length === MARKETS.length) return new Set();
  const out = new Set();
  for (const m of off) {
    if (m === 'ml') { out.add('home_ml'); out.add('away_ml'); out.add('draw'); }
    if (m === 'spread') { out.add('home_spread'); out.add('away_spread'); }
    if (m === 'total') { out.add('over'); out.add('under'); }
  }
  return out;
}

/** Same, loaded from the board for a sport. */
async function mutedSidesFor(supabase, sport) {
  return mutedSides(await publishFlags(supabase, sport));
}
...
/**
 * The narration-free result for a whole-sport shadow read (owner
 * 2026-09-14: shadow sports compute and grade into the ledger without
 * a Claude call until go-live, because NCAAF narration alone breached
 * the daily cost ceiling three Saturdays running while publishing
 * nothing). Same
 * shape analyzeGame returns, zero tokens, the math pick intact.
 */
function shadowNarration(mathPick, factors) {
  const adjustments = Array.isArray(factors?.adjustments) ? factors.adjustments : [];
  const keyFactors = adjustments
    .filter(a => a && a.factor != null && Number.isFinite(Number(a.impact)))
    .sort((a, b) => Math.abs(Number(b.impact)) - Math.abs(Number(a.impact)))
    .slice(0, 3)
    .map(a => `${a.factor} ${Number(a.impact) >= 0 ? '+' : ''}${(Number(a.impact) * 100).toFixed(1)}pp`);
  return {
    analysis_snippet: 'Shadow read. The model priced this game and grades it into the shadow ledger. Narration starts at go-live.',
...
module.exports = { marketOfSide, flagsFromRows, publishFlags, marketPublishOpen, mutedSides, mutedSidesFor, sportIsShadow, publishFlagsAll, shadowNarration, _resetCache, CODE_SHADOW_SPORTS, MARKETS };
