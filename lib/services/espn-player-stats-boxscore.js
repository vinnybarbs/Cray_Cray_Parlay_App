...
class ESPNPlayerStatsBoxScore {
  constructor(supabase) {
    ...
    this.baseUrl = 'https://site.web.api.espn.com/apis/site/v2/sports';
    this.cacheHours = 12;
  }
   ...
   * FIX: Combine multiple stat groups (passing, rushing, receiving) into ONE game entry
   */
  extractPlayerStatsFromBoxScore(boxScore, targetPlayers, playerStats, sport) {
    ...
    // Parse box score structure - combine stat groups per player
    for (const team of boxScore.boxscore.players) {
      if (!team.statistics) continue;
      
      // Track players in this game to combine stat groups
      const gamePlayersMap = new Map(); // playerName -> combined stats
      ...
      for (const statGroup of team.statistics) {
        ...
        for (const athlete of statGroup.athletes) {
          ...
          if (targetName) {
            ...
            // Get or create combined stats for this player in this game
            if (!gamePlayersMap.has(targetName)) {
              gamePlayersMap.set(targetName, {
                gameDate,
                opponent: this.getOpponent(boxScore, team.team?.id),
                passing_yards: 0,
                passing_tds: 0,
                interceptions: 0,
                rushing_yards: 0,
            ...
            // Parse this stat group and MERGE into existing stats
            const groupStats = this.parseAthleteStats(athlete, statGroup, sport);
            if (groupStats) {
              const combined = gamePlayersMap.get(targetName);
              
              // Merge stats (add non-zero values)
              Object.keys(groupStats).forEach(key => {
                if (key !== 'gameDate' && key !== 'opponent') {
                  const value = parseFloat(groupStats[key]) || 0;
                  if (value > 0) {
      ...
      // Add combined game stats to player history
      for (const [targetName, combinedStats] of gamePlayersMap) {
        if (!playerStats[targetName]) {
          playerStats[targetName] = [];
        }
        playerStats[targetName].push(combinedStats);
      }
    }
  }
  ...
  parseAthleteStats(athlete, statGroup, sport) {
    ...
    const statGroupName = statGroup.name || 'unknown';
    const playerName = athlete.athlete?.displayName || 'unknown';
    
    // DEBUG: Log what ESPN is sending
    if (playerName.includes('Henry') || playerName.includes('Mahomes') || playerName.includes('Kelce')) {
      console.log(`\n🔍 DEBUG ${playerName} [${statGroupName}]:`);
      console.log(`   Labels: ${JSON.stringify(statLabels)}`);
      console.log(`   Values: ${JSON.stringify(statValues)}`);
    }
    ...
    switch (sport) {
      case 'NFL':
        return this.parseNFLStats(stats, statGroupName);
      case 'NBA':
  ...
  parseNFLStats(stats, statGroupName) {
    // Use stat group name to determine which stats to parse
    const groupLower = (statGroupName || '').toLowerCase();
    ...
    // Parse based on stat group type
    if (groupLower.includes('passing')) {
      parsed.passing_yards = parseFloat(stats.yds || stats.pass_yds || 0);
      parsed.passing_tds = parseInt(stats.td || stats.pass_td || 0);
      parsed.interceptions = parseInt(stats.int || 0);
    } else if (groupLower.includes('rushing')) {
      parsed.rushing_yards = parseFloat(stats.yds || stats.rush_yds || 0);
      parsed.rushing_tds = parseInt(stats.td || stats.rush_td || 0);
    } else if (groupLower.includes('receiving')) {
      parsed.receptions = parseInt(stats.rec || 0);
      parsed.receiving_yards = parseFloat(stats.yds || stats.rec_yds || 0);
      parsed.receiving_tds = parseInt(stats.td || stats.rec_td || 0);
      parsed.targets = parseInt(stats.tgt || 0);
    } else {
      // Fallback: try to detect by stat keys
      if (stats.c_att || stats.comp || stats.att) {
        // Passing
        parsed.passing_yards = parseFloat(stats.yds || 0);
        parsed.passing_tds = parseInt(stats.td || 0);
        parsed.interceptions = parseInt(stats.int || 0);
      } else if (stats.car || stats.carries) {
        // Rushing
        parsed.rushing_yards = parseFloat(stats.yds || 0);
        parsed.rushing_tds = parseInt(stats.td || 0);
      } else if (stats.rec || stats.tgt) {
        // Receiving
        parsed.receptions = parseInt(stats.rec || 0);
        parsed.receiving_yards = parseFloat(stats.yds || 0);
        parsed.receiving_tds = parseInt(stats.td || 0);
        parsed.targets = parseInt(stats.tgt || 0);
      }
    }
  ...
  formatStatsForAI(playerName, stats, sport) {
    ...
    switch (sport) {
      case 'NFL':
        // Determine player type by which stats are > 0
        const passYds = parseFloat(stats.passing_yards) || 0;
        const rushYds = parseFloat(stats.rushing_yards) || 0;
        const recYds = parseFloat(stats.receiving_yards) || 0;
        const recs = parseFloat(stats.receptions) || 0;
        
        if (passYds > 50) {
          // QB
          summary += `${stats.passing_yards} pass yds/game, ${stats.passing_tds} pass TDs/game`;
          if (rushYds > 10) summary += `, ${stats.rushing_yards} rush yds/game`;
        } else if (rushYds > 20) {
          // RB
          summary += `${stats.rushing_yards} rush yds/game`;
          if (recs > 1) summary += `, ${stats.receptions} rec/game, ${stats.receiving_yards} rec yds/game`;
        } else if (recYds > 10 || recs > 0) {
          // WR/TE
          summary += `${stats.receptions} rec/game, ${stats.receiving_yards} rec yds/game`;
          if (parseFloat(stats.receiving_tds) > 0) summary += `, ${stats.receiving_tds} TDs/game`;
        } else {
          summary += `No significant stats`;
        }
        break;
