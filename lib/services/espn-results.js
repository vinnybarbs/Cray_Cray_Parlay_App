 ...
 * as 1-0 / 0-1, and the upstream moneyline grader uses sign(homeScore - awayScore).
 */
...
// is plenty, since the cron only runs every 2 hours.
const UFC_EVENT_CACHE = new Map();
const UFC_CACHE_TTL_MS = 30 * 60 * 1000;

// ESPN's `dates=` param buckets by US Eastern game-day. suggestion.game_date
// is usually a date-only string ('2026-07-24') that IS already the game-day
// bucket — new Date() would read it as midnight UTC (the prior US evening),
// so a timezone conversion would shift it back a day. Use date-only strings
// verbatim; only genuine timestamps get converted to the Eastern game-day.
function espnDay(gameDateValue) {
  if (typeof gameDateValue === 'string') {
    const m = gameDateValue.match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if (m) return `${m[1]}${m[2]}${m[3]}`;
  }
  return sportDayCompact(new Date(gameDateValue));
}

// Shift a YYYYMMDD bucket by whole days (pure calendar math, no timezones).
function shiftDay(compact, days) {
  const d = new Date(Date.UTC(+compact.slice(0, 4), +compact.slice(4, 6) - 1, +compact.slice(6, 8) + days));
...
async function resolveTeamSport(suggestion) {
  ...
  const dateStr = espnDay(suggestion.game_date);
  const dayBeforeStr = shiftDay(dateStr, -1);
...
async function resolveTennis(suggestion) {
  const dateStr = espnDay(suggestion.game_date);
  const dayBeforeStr = shiftDay(dateStr, -1);
  const dayAfterStr = shiftDay(dateStr, 1);
...
async function resolveUfc(suggestion) {
  ...
  const dateStr = espnDay(suggestion.game_date);
  const dateCandidates = [dateStr, shiftDay(dateStr, -1), shiftDay(dateStr, 1)];
