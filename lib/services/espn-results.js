...
async function resolveTeamSport(suggestion) {
  ...
  // The pick's game_date is a real kickoff instant, and so is ESPN's
  // event.date. Same-game identity is the instant match (±3h), never the
  // scoreboard bucket: buckets are calendar days and a same-pair series
  // game one night over sits in an adjacent bucket with the same teams.
  // Both buckets get searched because an evening US game can land in
  // either UTC bucket.
  const SAME_GAME_WINDOW_MS = 3 * 60 * 60 * 1000;
  let game = null;
  for (const d of [dateStr, dayBeforeStr]) {
    ...
    game = (result.events || []).find(event => {
      ...
      if (!ok && !reversed) return false;
      const eventTime = event.date ? new Date(event.date).getTime() : NaN;
      return Number.isFinite(eventTime)
        && Math.abs(eventTime - gameDate.getTime()) <= SAME_GAME_WINDOW_MS;
    });
    if (game) break;
  }
