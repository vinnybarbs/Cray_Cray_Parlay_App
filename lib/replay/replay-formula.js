...
const { mutedSidesFor } = require('../services/publish-markets.js');
const { pricePenaltyPp } = require('../services/price-penalties.js');
const { edgeTier } = require('../services/pick-grader.js');
const { regularSeasonFloor } = require('../services/season-floor.js');
...
async function standingsAsOf(real, teamName, sport, asOfDate) {
  ...
  const lookback = siteDate(new Date(new Date(`${asOfDate}T12:00:00Z`).getTime() - SEASON_LOOKBACK_DAYS * 24 * 3600 * 1000));
  const floor = regularSeasonFloor(sport, new Date(`${asOfDate}T12:00:00Z`));
  const since = floor && floor > lookback ? floor : lookback;
  let rows = [];
  try {
...
/**
 * As-of league venue bump: every team's standings row rebuilt from the
 * finals before asOfDate, then the calculator's own league mean over
 * them. Cached per sport and date.
 */
const _bumpCache = new Map();
async function leagueVenueBumpAsOf(real, sport, asOfDate, calc) {
  const cacheKey = `${sport}|${asOfDate}`;
  if (_bumpCache.has(cacheKey)) return _bumpCache.get(cacheKey);
  const lookback = siteDate(new Date(new Date(`${asOfDate}T12:00:00Z`).getTime() - SEASON_LOOKBACK_DAYS * 24 * 3600 * 1000));
  const floor = regularSeasonFloor(sport, new Date(`${asOfDate}T12:00:00Z`));
  const since = floor && floor > lookback ? floor : lookback;
  let rows = [];
  ...
  const teams = new Set();
  for (const g of rows) { if (g.home_team_name) teams.add(g.home_team_name); if (g.away_team_name) teams.add(g.away_team_name); }
  const standings = [];
  for (const t of teams) {
    const row = standingsFromGames(rows.filter(g => g.home_team_name === t || g.away_team_name === t), t);
    if (row) standings.push(row);
  }
  const out = calc._leagueVenueBumpFromRows(standings, sport);
  _bumpCache.set(cacheKey, out);
  return out;
}
...
/**
 * Current: live calculator, live band map, raw gate, price rails, headline
 * side. `overrides` is a map of dial name to value tried in place of the
 * board (the counterfactual sweep); chalk_penalty_pp and
 * longshot_penalty_pp reach the price rails, everything else reaches the
 * calculator's dial accessor.
 */
async function currentPick(real, game, sportDisplay, asOfDate, overrides = {}) {
  const calc = new LiveCalculator(asOfSupabase(real, asOfDate));
  calc.dialOverrides = overrides;
  calc.getStandingsSnapshot = (team, sport) => standingsAsOf(real, team, sport, asOfDate);
  calc.getLeagueVenueBump = (sport) => leagueVenueBumpAsOf(real, sport, asOfDate, calc);
  let edgeData = await calc.calculateEdge(game);
  if (!edgeData) return { edgeData: null, pick: null };
  edgeData = await bandCalibration.applyToEdgeData(edgeData, sportDisplay);
  // Best side by the scaled claim among the markets whose publish dial
  // is open (a muted market never headlines live, so it never headlines
  // here), no floor: the raw gate below is the only publication test.
  const best = calc.pickBestSide(edgeData, { minEdgePp: 0, excludeSides: await mutedSidesFor(real, sportDisplay) });
  if (!best) return { edgeData, pick: null };
  const raw = edgeData.edgesRaw?.[best.side];
  // min_gate_pp (variant only): a higher raw floor than the 2pp gate.
  // The gate reads the raw claim only (directive 25 as amended
  // 2026-09-21): the multiplier sizes the label, never publication.
  const minGate = overrides.min_gate_pp != null ? Number(overrides.min_gate_pp) : 2;
  if (raw == null || raw * 100 < minGate) return { edgeData, pick: null, gated: true };
  // A scaled claim under 2pp publishes at the Lean floor, as live.
  const pp = Math.max(2, Math.round(best.signedEdge * 1000) / 10);
  const railDials = {};
  if (overrides.chalk_penalty_pp != null) railDials.chalkPp = Number(overrides.chalk_penalty_pp);
  if (overrides.longshot_penalty_pp != null) railDials.longshotPp = Number(overrides.longshot_penalty_pp);
  const price = priceForSide(game, best.side);
  // rails_before_gate (variant only): the price rail comes off BEFORE the
  // publish gate and without the Lean floor, so a chalk claim that the
  // rail eats does not publish at all. The live pipeline gates first and
  // floors the rail at 2pp, which is why chalk_penalty_pp alone never
  // changes which picks publish (dial sweep 2026-09-11).
  if (Number(overrides.rails_before_gate) === 1) {
    const o = Number(String(price ?? '').replace(/[^0-9-]/g, ''));
    const chalkPp = railDials.chalkPp ?? 3, longshotPp = railDials.longshotPp ?? 6;
    let rail = 0;
    if (Number.isFinite(o) && o < 0 && o <= -150) rail = chalkPp;
    else if (Number.isFinite(o) && o >= 300) rail = longshotPp * (o / 300);
    if (pp - rail < 2) return { edgeData, pick: null, gated: true };
  }
  const priced = pricePenaltyPp(pp, price, railDials);
...
/**
 * Variants: name:kind[:dial=value,dial=value]; several separated by ';'.
 *   "july:july;base:current;chalk5:current:chalk_penalty_pp=5"
 */
function parseVariants(text) {
  const out = [];
  for (const part of String(text || '').split(';').map(s => s.trim()).filter(Boolean)) {
    const [name, kind = name, dialText = ''] = part.split(':');
    if (!['july', 'current'].includes(kind)) continue;
    const dials = {};
    for (const kv of dialText.split(',').map(s => s.trim()).filter(Boolean)) {
      const [k, v] = kv.split('=');
      if (k && v != null && Number.isFinite(Number(v))) dials[k.trim()] = Number(v);
    }
    out.push({ name: name.trim(), kind, dials });
  }
  return out;
}

function runVariant(variant, real, game, asOf, sport) {
  return variant.kind === 'july' ? julyPick(real, game, asOf) : currentPick(real, game, sport, asOf, variant.dials || {});
}
 ...
 */
async function runReplay(supabase, { sport = 'MLB', days = 30, formulas = ['july', 'current'], variants = null, runId = null, concurrency = 4, log = () => {} } = {}) {
  const variantList = Array.isArray(variants) && variants.length
    ? variants
    : formulas.map(f => ({ name: f, kind: f, dials: {} }));
  ...
  const summary = { run_id: run, sport, days, games_analyzed: rows.length, games_with_results: games.length, variants: variantList.map(v => ({ name: v.name, kind: v.kind, dials: v.dials })), formulas: {} };
  for (const v of variantList) summary.formulas[v.name] = { picks: 0, won: 0, lost: 0, push: 0, units: 0, no_edge: 0, gated: 0, errors: 0, by_tier: {} };
  ...
  await mapPool(games, concurrency, async (g) => {
    for (const v of variantList) {
      ...
      try {
        const out = await runVariant(v, supabase, g.game, g.asOf, sport);
...
module.exports = { runReplay, parseVariants, gameFromAnalysis, gradeSide, unitsFor, priceForSide, asOfSupabase, pickText, siteDate, standingsFromGames, standingsAsOf, leagueVenueBumpAsOf, JULY_MULTIPLIERS };
