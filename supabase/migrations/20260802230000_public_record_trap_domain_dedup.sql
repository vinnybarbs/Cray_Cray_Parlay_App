-- mv_public_record v3: picks and traps dedupe in SEPARATE domains.
--
-- v2 deduped to one row per (home, away, game_date) across ALL auto_digest
-- sessions. Once traps publish independently of picks (same game can carry
-- a pick on one side and a trap on another, pre-analyze 2026-08-02), that
-- dedup would silently drop whichever row revised earlier. The DISTINCT ON
-- now includes (tier = 'Trap') so one pick row AND one trap row per game
-- both survive. Trap rows still appear only in the tier dimension: the
-- picks CTE below excludes them from overall/sport/bet_type, unchanged
-- from v2. Applied to prod 2026-08-02 via MCP.

DROP MATERIALIZED VIEW IF EXISTS public.mv_public_record;

CREATE MATERIALIZED VIEW public.mv_public_record AS
WITH base AS (
  SELECT DISTINCT ON (s.home_team, s.away_team, s.game_date, (s.tier::text = 'Trap'))
  ...
  ORDER BY s.home_team, s.away_team, s.game_date, (s.tier::text = 'Trap'),
