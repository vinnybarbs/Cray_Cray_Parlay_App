  ...
  try {
    ...
    const todaysGamesResult = await safeQuery(async () => {
      const { data, error } = await supabase
        .from('game_analysis')
        .select(
          'game_key, home_team, away_team, game_date, edge_score, recommended_pick, recommended_side, recommended_odds, analysis_snippet, key_factors, spread, total, moneyline_home, moneyline_away, spread_home_price, spread_away_price, over_price, under_price, home_record, away_record, home_ranking, away_ranking, sport, analysis_version, edge_movement, what_changed, edges, edges_raw, trap_calls, calc_home_prob, calc_away_prob, model_used'
        )
        .eq('stale', false)
  ...
  try {
    ...
    const gameAnalysisResult = await safeQuery(async () => {
      const { data, error } = await supabase
        .from('game_analysis')
        .select(
          'home_team, away_team, game_date, sport, edge_score, edge_movement, analysis_snippet, key_factors, what_changed, prior_analysis, analysis_version, recommended_pick, recommended_side, recommended_odds, spread, total, moneyline_home, moneyline_away, spread_home_price, spread_away_price, over_price, under_price, home_record, away_record, edges, edges_raw, trap_calls, model_used'
        )
