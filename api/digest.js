  ...
  try {
    ...
    const gameAnalysisResult = await safeQuery(async () => {
      const { data, error } = await supabase
        .from('game_analysis')
        .select(
          'home_team, away_team, game_date, sport, edge_score, edge_movement, analysis_snippet, key_factors, what_changed, prior_analysis, analysis_version, recommended_pick, recommended_side, recommended_odds, spread, total, moneyline_home, moneyline_away, home_record, away_record, edges, edges_raw, trap_calls, model_used'
        )
