 ...
 *
 * One post each morning with everything live on today's board as a set
 * of embeds, one per tier plus Legs and Traps, no site link (owner
 * 2026-09-21: the unfurled preview crowded the channel). The footer says
 * the board revises until lock, and the Sharp Take promotion alerts
 * cover the intraday changes, so together the board channel is: full
 * picture in the morning, pings when the money tier moves.
 *
...
const { sendDiscordEmbeds, embedsFromLines, TIER_COLOR, BRAND_COLOR } = require('../../lib/services/discord-alerts.js');
const { siteDay } = require('../../shared/site-day.js');

const TIER_ORDER = ['Sharp Take', 'Strong Play', 'Play', 'Lean'];
const FOOTER = 'Tiers revise with prices until lock. Promotions into Strong Play or Sharp Take ping here as they happen.';
...
/** Pure formatter: the board as embeds, testable without Discord. */
function formatMorningBoard(rows, dateLabel, gimmes = []) {
  ...
  for (const tier of TIER_ORDER) {
    ...
    anything = true;
    const lines = group.sort((a, b) => (b.edge_pp ?? 0) - (a.edge_pp ?? 0)).map(pickLine);
    embeds.push(...embedsFromLines(`${tier}${group.length > 1 ? 's' : ''}`, lines, { color: TIER_COLOR[tier] }));
  }
  const legs = byTier.get('Leg') || [];
  if (legs.length) {
    anything = true;
    const lines = legs.sort((a, b) => (b.model_prob ?? 0) - (a.model_prob ?? 0)).map(r => {
      const prob = r.model_prob != null ? `${Math.round(r.model_prob * 100)}% to hit` : null;
      return `• ${[r.pick, matchupOf(r), r.sport, prob, gameTimeMt(r.game_date)].filter(Boolean).join(' · ')}`;
    });
    embeds.push(...embedsFromLines('Legs · the gimmes, 65% or better to hit, parlay material', lines, { color: TIER_COLOR.Leg }));
  }
  ...
  if (!hasBetSignal && (gimmes || []).length) {
    anything = true;
    const lines = ['These are not plays, but the market prices them as the day\u2019s highest percenters and the model agrees with the price:'];
    ...
    embeds.push(...embedsFromLines('No Sharp Take edge on the board today', lines, { color: TIER_COLOR.Lean }));
  }
  const traps = byTier.get('Trap') || [];
  if (traps.length) {
    anything = true;
    const lines = traps.sort((a, b) => (a.edge_pp ?? 0) - (b.edge_pp ?? 0)).map(pickLine);
    embeds.push(...embedsFromLines('Traps · the advice is to fade these sides', lines, { color: TIER_COLOR.Trap }));
  }
  const header = {
    title: `🦅 TrapHawk board · ${dateLabel}`,
    description: anything
      ? `${(rows || []).length} live reads on the board.`
      : 'Quiet board so far. Sweeps run all day and promotions ping here when a Strong Play or Sharp Take lands.',
    color: BRAND_COLOR,
  };
  const all = [header, ...embeds];
  all[all.length - 1].footer = { text: FOOTER };
  return all;
}
