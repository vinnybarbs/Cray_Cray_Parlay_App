...
- `nfl_injury_reports` (since 2026-09-14): the official NFL practice and game status report per player per week from the nflverse injuries release, synced every 6 hours by `sync-nfl-injuries`. `report_status` is the Friday designation (Out, Doubtful, Questionable, null for none), `practice_status` the last participation (Full, Limited, Did Not Participate), keyed (season, week, team, player_key) with team as the nflverse club code (`teamAbbr` in lib/services/nfl-inactives.js maps full names). It is narration context for NFL (pre-analyze reads it before news_cache) and the replay input for a practice participation factor after the freeze (build_queue 35). The injury factor's MATH still reads the ESPN status list. `nfl_depth_charts`: since 2026-09-15 a rolling 28 day window of each club's depth chart snapshots from the same release (unique on team, snapshot_at and slot, only snapshots newer than stored are written each run), `pos_rank` 1 is the starter for a slot; `nfl_depth_rank_floor` (view) is each player's best rank over the window, read by the injury factor's depth gate in lib/services/football-injuries.js (rank 1 full position cost, rank 2 times `injury_depth2_weight`, rank 3 and below zero, unlisted times `injury_depth_unknown_weight`), and the report lines in `edge_factors.injuryReport` carry `depth_rank` and `depth_weight` so a read shows what each absence cost. `game_analysis.edge_factors.injuryReport` ({home, away} lists of {player, position, status}) is the report an NFL read was priced on; `watch-nfl-inactives` (every 15 minutes, 3 hours before kickoff) diffs ESPN's game summary against it, and a player newly Out marks the analysis stale and re-runs pre-analyze-NFL, so a late scratch revises the pick through the same formula and leaves a tier_history entry. Its cron_job_logs details carry the scratches it found.
