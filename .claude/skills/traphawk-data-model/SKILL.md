...
`prop_reads` is the shadow record for NFL player props, one row per (event, market, player), written by `api/cron/analyze-nfl-props.js` twice daily and graded by the same endpoint with `?mode=grade` against `nfl_player_game_stats` (nflverse, the settlement truth, synced Tuesdays). The formula (`lib/services/prop-edge.js`) has the same shape as every team market: anchor at the devigged consensus over price at the median book line (`player_props`, 8 books), model from the player's own game log, damp the disagreement by `prop_claim_damp` times games over `prop_min_games`, band the pp. Every weight is an `NFL_props` row in `sport_dials`. Five over/under markets are modeled (pass yds, pass TDs, rush yds, rec yds, receptions); anytime TD is not. Nothing publishes: no ai_suggestions row, no digest tile, no alert, so `prop_reads` never touches mv_public_record. Judge it like any shadow sport: graded reads at 2pp+ against the anchor, won minus anchor implied, by market and by band. Since 2026-09-14 every row also carries a v2 shadow read (`v2_mean`, `v2_prob`, `v2_edge_pp`, `v2_side`, `v2_tier`, `v2_outcome`, `v2_factors`): the v1 baseline scaled by the opponent's allowance per game against league average (`prop_opp_weight`), the team's implied points from the median spread and total against the slate average (`prop_env_weight`), and wind on the passing markets outdoors (`prop_wind_weight`), skipped when Sleeper or the official report lists the player out (`v2_factors.skipped`). The grader settles `v2_outcome` from the same stat line. `actual_outcome` = `void` means the player had no stat line after the week's file landed (a book void); exclude void from every record. The first graded week (2026-09-14, v1 only) went 192-170 with a flat ladder and negative price CLV, so read the v2 columns as the candidate and the v1 columns as the frozen regime.
...
| Leg | model probability at or above the `leg_prob_floor` dial (since 2026-09-21: `__all__` 0.75, NFL 0.80, MLB 0.70; owner: near sure things only, the flat 0.65 let every NFL -200 favorite in) but no 2pp edge (payout too thin). Tracked for parlay building |

Edges are scaled by `edge_calibration` multipliers (keys `<Sport>:<market>`, `<Sport>`, `__global__`) before tiering, and since 2026-09-21 (directive 25 as amended, owner: no haircut out of fear) EVERY multiplier is 1: they are dials, no weekly refit (edge_calibration_weekly and both band refit jobs are unscheduled), and one leaves 1 only through the three test sweep or an owner call with a model_weight_changes row. The publish gate binds on the raw claim ALONE (2pp), the multiplier sizes only the label, and a scaled claim under 2pp publishes at the Lean floor. A market mute is `publish_<market>` 0 on the dial board (MLB and NFL totals since 2026-09-21), never a zero multiplier, and `promote_ready_markets()` lifts a mute by flipping that dial with the multiplier at 1. The band maps (`edge_band_calibration`, `edge_band_calibration_raw`) are identity and lib/services/band-calibration.js is identity in code, so the tier is the raw claim times the multiplier minus the price rails and nothing else.  The bands are buckets with floors in `bucket_targets` (pp delivered over break even at the price: Lean 2, Play 4, Strong Play 7, Sharp Take 10, sport override allowed); `bucket_scorecard(p_days, p_min_n, p_regime_floor)` scores every sport and `__all__` per bucket on the public record's population, honoring `band_fit_floors`, and `file_bucket_scorecard()` (cron bucket-scorecard-weekly, Mondays 06:50 UTC) files it to agent_reports as agent bucket-scorecard with the next sweep target. `replay_buckets()` gives the same table per replay variant and `file_replay_candidates` prints it on every candidate. Live multipliers are read from the table, never from this file: `select key, multiplier, measured_k, source, updated_at from edge_calibration order by key;` (the ops check found this paragraph stale on three keys on 2026-09-10, so numbers no longer live here). Decisions by date, for context only: MLB ml floored at 0.25 on 2026-08-24, MLB spreads re-entered 2026-08-10, MLB totals re-entered on probation 2026-08-31 under the band aware map (PR 110), Tennis:ml refit weekly since 2026-09-07, UFC:ml promoted out of shadow 2026-08-25 (claims average 0.35pp, so UFC is mostly a board, leg, and trap product, not a pick engine), NFL seeded 2026-09-09 at go-live with totals muted. Legs are GIMMES only: calibrated model probability 65% or better, no loosening (a market-anchored second path shipped and was reverted the same day, 2026-08-25, at owner direction). The quiet-day Discord content comes from the morning board's presentation-only high-percenter list (fairImplied 60%+ favorites the model agrees with, from game_analysis, never published or graded). Discord (owner redesign 2026-09-21) is three channels, every post an embed with no site link: board (DISCORD_WEBHOOK_URL: the 08:00 MT morning board and Strong Play or Sharp Take entry alerts), receipts (DISCORD_WEBHOOK_RECEIPTS_URL: 07:30 MT card of yesterday's settled picks by tier plus the 3, 7 and 30 day record, every number from mv_public_record), and model (DISCORD_WEBHOOK_MODEL_URL: an hourly feed of new model_weight_changes rows and new or amended directives, cursor in `discord_feed_cursor`, plus the Monday 10:00 MT dial board embed, one field per sport with multipliers, the week's moves and the bucket floors). A channel without its own webhook falls back to the board one. Posters: api/cron/discord-morning-board.js, discord-receipts.js, discord-model-feed.js; cron_job_logs rows discord-morning-board, discord-receipts, discord-model-feed, discord-dial-board. A `reprice-pending-picks` cron demotes pending Play moneyline picks to Lean when the market fades their side by 1pp of implied probability inside 90 minutes of first pitch.
...
- `nfl_injury_reports` (since 2026-09-14): the official NFL practice and game status report per player per week from the nflverse injuries release, synced every 6 hours by `sync-nfl-injuries`. `report_status` is the Friday designation (Out, Doubtful, Questionable, null for none), `practice_status` the last participation (Full, Limited, Did Not Participate), keyed (season, week, team, player_key) with team as the nflverse club code (`teamAbbr` in lib/services/nfl-inactives.js maps full names). It is narration context for NFL (pre-analyze reads it before news_cache) and the replay input for a practice participation factor after the freeze (build_queue 35). The injury factor's MATH still reads the ESPN status list. `nfl_depth_charts`: since 2026-09-15 a rolling 28 day window of each club's depth chart snapshots from the same release (unique on team, snapshot_at and slot, only snapshots newer than stored are written each run), `pos_rank` 1 is the starter for a slot; `nfl_depth_rank_floor` (view) is each player's best rank over the window, read by the injury factor's depth gate in lib/services/football-injuries.js (the position cost times a per position ladder share for the rank: only the starting quarterback, WR1 through WR4 tapering, one starter per line slot, half for the second back, corner or lineman; unlisted times `injury_depth_unknown_weight`), and the report lines in `edge_factors.injuryReport` carry `depth_rank` and `depth_weight` so a read shows what each absence cost. `game_analysis.edge_factors.injuryReport` ({home, away} lists of {player, position, status}) is the report an NFL read was priced on; `watch-nfl-inactives` (every 15 minutes, 3 hours before kickoff) diffs ESPN's game summary against it, and a player newly Out marks the analysis stale and re-runs pre-analyze-NFL, so a late scratch revises the pick through the same formula and leaves a tier_history entry. Its cron_job_logs details carry the scratches it found.
- `admin_users` (since 2026-09-15): the admin allowlist, one row per email, seeded with every account at the time (owner: "add all users so far to admin ability"). `requireAdmin` in api/admin-dashboard.js passes an email on the ADMIN_EMAILS env list (the floor, so the owner can never be locked out) or on this table, cached five minutes. `GET /api/admin/dials?sport=X&days=N` (api/admin-dials.js, page /admin/dials) is the read only dial board per market: the merged `sport_dials` board with the scope that won, the rails, the publication flags, the `edge_calibration` multipliers, the raw band map, the last ten `model_weight_changes`, and every `game_analysis` read in the window with its factor stack, attached picks and `shadow_reads_graded` rows (NFL_props returns `prop_reads` with the v1 and v2 columns). It writes nothing: a dial moves only through sport_dials plus model_weight_changes under directives 7, 8 and 21.
- `lookback_reads` (since 2026-09-15, build_queue 40): one row per settled anchored football read, written by `run_football_lookback(p_days)` (cron football-lookback-weekly, Tuesdays 09:00 MT): `home_won`, `anchor_toward_winner`, `net_toward_winner`, `market_move_home_pp` (the h2h close against the read price, positive toward home), `close_toward_winner`, and `factors` [{family, factor, impact_pp, toward_winner, toward_close, decisive}] where decisive means removing that factor alone would have flipped the side. The function files an agent_reports row from agent football-lookback with the per sport headline, the per factor family table and the misses. `band_fit_floors`: per sport regime line honored by `refresh_edge_band_calibration_raw` (NFL and NCAAF 2026-09-16 after the depth gate and seed_weight changes); rows before a sport's floor never enter its band fit or the `__all__` pool.
...
Whether a (sport, market) reaches the graded record is three dials on `sport_dials`: `publish_ml`, `publish_spread`, `publish_total`, 1 or 0 per sport (`__all__` 1; the shadow sports NCAAF, EPL, MLS, Soccer, World Cup, Champions League, Copa America, Euros start at 0 on all three). The SHADOW_SPORTS constants in code are only the fail-soft mirror. `/api/digest` sends `publishMarkets` and the board withholds a shadow market's read (src/lib/publish-flags.js). A mute is the same dial at 0 for a sport that otherwise publishes (MLB and NFL totals since 2026-09-21); the zero multiplier mute is gone. `shadow_reads_graded` and `shadow_market_readiness()` are the shadow ledger; `promote_ready_markets()` (daily) opens a market that clears the bar; `file_replay_candidates()` (Sunday sweep) files dial candidates in build_queue. `home_margin_shift` is the MLB spread transform dial.
