...
Math picks the side, Claude narrates. `api/cron/pre-analyze-games.js` computes per-side edges (edge calculator for team sports, dedicated models in `docs/models/` for tennis, UFC, soccer), then calls Sonnet only to write the analysis. The LLM never chooses the side. Shadow sports (`SHADOW_SPORTS`: the soccer family plus NFL and NCAAF) store edges and analyses but never publish picks. `SOCCER_1X2_SPORTS` is a separate set that answers which model prices a game, `SHADOW_SPORTS` answers whether it publishes. Tennis left the shadow list 2026-08-10 and publishes through the ladder at a 0.50 multiplier, and UFC left it 2026-08-25 on 81 graded shadow reads at 48-33 with measured k 4.43, so UFC picks are real but stay rare under the same 2pp gate. Neither is a shadow sport any more and neither is judged against the promotion bar. Football go-live is a deliberate flip at the openers, NFL 2026-09-10, and NCAAF was planned for 2026-08-29 but the flip was not taken, so NCAAF is still shadowed pending an owner decision. Both seed from preseason `market_shadow_calibration()` measured_k. Shadow promotion for player sports is judged by `shadow_model_readiness()`: 75 graded publishable reads, actual at or above fair implied, AND positive publishable units. Never a raw read count.
