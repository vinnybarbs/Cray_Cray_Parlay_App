-- Fix cron jobs to use correct net.http_post syntax
-- The pg_net extension creates functions in the 'net' schema, not 'pg_net'

-- 1. First, let's check what's available
SELECT schema_name FROM information_schema.schemata WHERE schema_name IN ('net', 'pg_net');

-- 2. Remove all the problematic cron jobs
DO $$
BEGIN
    -- Remove jobs that are failing with pg_net errors
...
-- 3. Create working cron jobs using net.http_post (not pg_net.http_post)
SELECT cron.schedule(
    'refresh-odds-hourly-working',
    '0 * * * *',
    $$
...
SELECT cron.schedule(
    'refresh-intelligence-2hourly-working',
    '15 */2 * * *',
    $$
    SELECT net.http_post(
...
SELECT cron.schedule(
    'check-parlay-outcomes-30min-working',
    '*/30 * * * *',
    $$
    SELECT net.http_post(
