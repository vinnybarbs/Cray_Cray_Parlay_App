...
export default function YesterdayBoard() {
        ...
        <div className="bg-ink-950/60 rounded-sharp shadow-hairline overflow-hidden">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 px-4 py-2.5 bg-ink-950 font-mono text-[10px] uppercase tracking-[0.14em] text-ink-400">
            ...
            {s && s.total > 0 && (
              <>
                ...
                <span className={`tabular-nums ${s.units >= 0 ? 'text-signal-pos' : 'text-signal-neg'}`}>{s.units >= 0 ? '+' : ''}{s.units.toFixed(2)}u at 1u</span>
              </>
            )}
          ...
          {(!s || s.total === 0) ? (
            ...
            data.picks.map((p, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-3 px-4 py-2.5 border-t border-ink-800">
